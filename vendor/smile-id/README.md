...
2026-09-23 capture/recovery audit: the isolated frame connect-src needs data: because the official DotLottie animations fetch bundled data URLs. A browser regression verifies painted animation pixels, not merely successful SDK initialization. The default smile flow explicitly prompts for a slightly open mouth. Presentation uses an undistorted 311:418 oval with stronger strokes and hides only the visual landmark mesh. Active camera tracks request a sharper 1280px preview through applyConstraints, preserving aspect ratio; unsupported devices retain their original working stream. Provider image sizes/encoding and biometric thresholds remain unchanged.
