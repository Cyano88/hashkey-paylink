...
const mocks={
 ...
 pocketStockNotificationsClient:`export const stockNotificationsRequest=async()=>{if(window.failNotices)throw Error('fixture');return {notices:[{id:window.owner,hash:'hash',at:1,transfer:{from:'sender',to:'wallet',direction:'in',amount:'1',symbol:'NVDAx'}}],requests:[]}}`,
 pocketXPayClient:`export const xpayRequest=async()=>{if(window.failXpay)throw Error('fixture');return {payments:[]}}`,
 pocketRefresh:`export const registerPocketRefreshHandler=f=>{window.refresh=f;return()=>{}}`,
}
const contents=`import React from 'react';import{createRoot}from'react-dom/client';import Activity from './src/pocket/components/PocketStockActivity';const root=createRoot(document.getElementById('root'));window.renderActivity=key=>root.render(<Activity key={key} wallet={{address:'wallet'}}/>);window.owner='a';window.failXpay=true;window.renderActivity(1)`
