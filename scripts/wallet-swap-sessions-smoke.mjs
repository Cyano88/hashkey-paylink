...
const h=createWalletSwapSessionHandlers({env,hasStore:()=>true,policy:async()=>({partnerId:project,merchantName:'Fixture',environment:'live',checkoutMode:'human',capabilities:['arc_agreements','xstocks_agreements']}),projectEnabled:async()=>projectEnabled,identity:async()=>({userId:who}),read:async k=>structuredClone(store.get(k)),mutate:async(k,fn)=>{const r=await fn(structuredClone(store.get(k)));store.set(k,structuredClone(r));return r},dispatch:async(r,req,res)=>{dispatches.push({record:r,method:req.method,body:req.body,query:req.query});return res.json({ok:true,owner:r.userId})}})
