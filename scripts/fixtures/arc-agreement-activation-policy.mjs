...
const REVIEWED_ARC_AGREEMENT_FACTORY = '0x7777777777777777777777777777777777777777'
const REVIEWED_ARC_AGREEMENT_OPERATOR = '0x8888888888888888888888888888888888888888'
...
const env = {
  ...
  ARC_AGREEMENT_FACTORY_ADDRESS_MAINNET: REVIEWED_ARC_AGREEMENT_FACTORY,
  ARC_AGREEMENT_OPERATOR_ADDRESS_MAINNET: REVIEWED_ARC_AGREEMENT_OPERATOR,
  ARC_AGREEMENT_OPERATOR_WALLET_ID_MAINNET: 'e3fe3e85-1111-4111-8111-11111111d4d9',
  ...
  CIRCLE_API_KEY_ARC_MAINNET: 'LIVE_API_KEY:test-id:test-secret',
  CIRCLE_ENTITY_SECRET_ARC_MAINNET: 'a'.repeat(64),
...
assert.throws(() => authorizeArcAgreementActivation({
  policy, draft, payer, activationTimestamp, env: { ...env, ARC_AGREEMENT_FACTORY_ADDRESS_MAINNET: payer },
}), /mainnet factory deployment has been reviewed/)
assert.throws(() => authorizeArcAgreementActivation({
  policy, draft, payer, activationTimestamp, env: { ...env, CIRCLE_API_KEY_ARC_MAINNET: 'TEST_API_KEY:id:secret' },
}), /Circle production LIVE_API_KEY/)
assert.throws(() => authorizeArcAgreementActivation({
...
assert.throws(() => authorizeArcAgreementActivation({ policy: {...policy, environment: 'test'}, draft, payer, activationTimestamp, env }), /live API key/)
assert.throws(() => authorizeArcAgreementActivation({ policy, draft, payer, activationTimestamp, env: {...env, CIRCLE_API_KEY_ARC_MAINNET: undefined, CIRCLE_API_KEY: 'LIVE_API_KEY:fixture:fixture'} }), /production LIVE_API_KEY/)
assert.throws(() => authorizeArcAgreementActivation({ policy, draft, payer, activationTimestamp, env: {...env, ARC_AGREEMENT_CONFIRMATION_BLOCKS: '1'} }), /at least 5/)
