...
const stubs={pocketBillsClient:`const available={enabled:true,environment:'live',airtimeEnabled:true};export class PocketBillsApiError extends Error{};export const cachedPocketBillsAvailability=()=>available;export const readPocketBillsAvailability=async()=>available;const intent=()=>({id:'bill-fixture',state:'quoted',quoteExpiresAt:Date.now()+60000,treasuryAddress:'0x'+'2'.repeat(40),amountUsdc:'1'});export const quotePocketAirtime=async()=>({intent:intent()});export const quotePocketData=quotePocketAirtime,quotePocketTv=quotePocketAirtime,quotePocketElectricity=quotePocketAirtime;export const preparePocketAirtime=async()=>intent();export const confirmPocketAirtime=async()=>{window.checks=(window.checks||0)+1;return new Promise((resolve,reject)=>{window.latePending=()=>resolve({...intent(),state:'pending'});window.lateFailure=()=>reject(new Error('Late response failure'))})};export const refreshPocketAirtime=async()=>({...intent(),state:window.delivered?'delivered':'pending'});export const readPocketDataCatalog=async()=>({services:[],variations:[]});export const verifyPocketBillCustomer=async()=>({});`,pocketEvmTransferClient:`export const executePocketEvmTransfer=async()=>{window.sends++;return {txHash:'0x'+'3'.repeat(64),status:'confirmed'}}`,pocketPaymentApproval:'export const registerPocketPaymentPreparer=()=>()=>{};',pocketRefresh:'export const registerPocketRefreshHandler=()=>()=>{};'}
...
try {for(const late of ['latePending','lateFailure']) {
 ...
 await page.waitForFunction(()=>!!window.latePending)
 // Another recovery reader/tab may consume the retry journal first.
 await page.evaluate(()=>{localStorage.clear();window.delivered=true})
 await page.clock.runFor(6_000)
 await page.waitForFunction(()=>window.bills.status==='successful',{},{timeout:2000})
 await page.evaluate(name=>window[name](),late)
 await page.clock.runFor(10_000)
 assert.equal(await page.evaluate(()=>window.bills.status),'successful')
 assert.equal(await page.evaluate(()=>window.bills.intent.state),'delivered')
 assert.equal(await page.evaluate(()=>window.sends),1)
 await page.close()
} console.log('PASS: late pending and failed responses cannot overwrite delivered success; no second send.')
