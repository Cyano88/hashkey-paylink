...
'../lib/pocketMigrationClient':`export async function pocketMigrationRequest(token,body){window.calls.push(body.action);if(body.action==='status')return {snapshot:window.snapshot};if(body.action==='activate')return {snapshot:{...window.snapshot,phase:'completed'}};if(body.action==='reconcile'){if(window.statusError)throw Error('Temporary status failure');return {state:'pending',snapshot:window.snapshot}};throw Error('Unexpected mutation')}`
};
...
try{
 ...
 await page.getByRole('button',{name:'Check progress',exact:true}).click();await page.getByText('Wallet update complete.',{exact:true}).waitFor();assert.equal(await page.getByRole('alert').count(),0);assert.equal(await page.getByRole('button').count(),0);assert.equal(await page.evaluate(()=>window.completed),true);assert.equal(await page.evaluate(()=>window.calls.filter(c=>c==='activate').length),1);assert.equal(await page.evaluate(()=>window.calls.some(c=>['start','resume','recover'].includes(c))),false);
 await page.close();
