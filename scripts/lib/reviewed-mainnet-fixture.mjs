...
export async function runReviewedMainnetFixture(fixture, originalUrl) {
  const productionRelease = requireArcMainnetAgreementRelease()
  assert.ok(Object.isFrozen(productionRelease))
  ...
  const result = await build({ stdin: { contents: source, resolveDir: scripts, sourcefile: fileURLToPath(fixture), loader: 'js' },
    ...
    plugins: [{name: 'synthetic-reviewed-release', setup(b) {
      b.onLoad({filter: /arc-mainnet-boundary\.ts$/}, async ({path}) => {
        ...
        const declaration = /}> \| null = Object\.freeze\(\{[\s\S]*?\}\)/
        assert.ok(declaration.test(source))
        return {contents: source.replace(declaration, "}> | null = Object.freeze({chainId: 5042, factory: '0x7777777777777777777777777777777777777777', operator: '0x8888888888888888888888888888888888888888'})"), loader:'ts', resolveDir:dirname(path)}
      })
    }}] })
  ...
  finally { globalThis.fetch = originalFetch; await unlink(output).catch(() => {}); assert.strictEqual(requireArcMainnetAgreementRelease(), productionRelease) }
}
