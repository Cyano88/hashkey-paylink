...
const create=()=>call(handlers.developer,{action:'create',subject:'hps-user-1',email:'user@example.com',challenge:hash(verifier)});
let r=await create();assert.equal(r.statusCode,201);const id=r.body.id,access=new URL('https://example.com'+r.body.connectPath).hash.slice(8);
...
r=await redeem();assert.equal(r.body.hashPayLinkUserId,'did:privy:user');assert.equal(r.body.subject,'hps-user-1');
