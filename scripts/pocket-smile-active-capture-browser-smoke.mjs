...
try {
 for (const method of ['biometric-kyc','doc-verification']) {
  ...
  if(process.env.SMILE_TEST_LOW_RES==='1')await page.addInitScript(()=>{MediaStreamTrack.prototype.applyConstraints=async()=>{throw new DOMException('Fixture unsupported resolution','OverconstrainedError')}})
  ...
  await button.click();await page.waitForFunction(()=>window.__captures>=3);await page.waitForFunction(()=>!!window.__pocketCameraQuality);if(process.env.SMILE_TEST_LOW_RES==='1')assert.equal(await page.evaluate(()=>window.__pocketCameraQuality.applied),false);else assert.ok((await page.evaluate(()=>window.__pocketCameraQuality.width))>=1280,'Higher resolution preview negotiated');await page.waitForTimeout(1000)
