...
try {
 for (const method of ['biometric-kyc','doc-verification']) {
  ...
  await page.addInitScript(()=>{window.__constraintCalls=0;const original=MediaStreamTrack.prototype.applyConstraints;MediaStreamTrack.prototype.applyConstraints=function(...args){window.__constraintCalls++;return original.apply(this,args)}})
  ...
  for(const width of [320,360,390,430]){await page.setViewportSize({width,height:792});const box=await page.locator('#capture-test .camera-preview-container').boundingBox();assert.ok(box.width>=Math.min(width-80,320)&&box.width<=320);assert.ok(box.x>=16&&box.x+box.width<=width-16);assert.equal(await page.locator('main').evaluate(e=>e.scrollWidth>e.clientWidth),false)}
  await button.click();await page.waitForFunction(()=>window.__captures>=3);assert.equal(await page.evaluate(()=>window.__constraintCalls),0,'Pocket must not renegotiate the active provider camera');const cameraTime=await page.locator('#capture-test .camera-video').evaluate(v=>v.currentTime);await page.waitForTimeout(300);assert.ok(await page.locator('#capture-test .camera-video').evaluate((v,t)=>v.currentTime>t,cameraTime),'Camera frames must continue advancing');await page.waitForTimeout(1000)
  const fitted=await page.locator('#capture-test .camera-video').evaluate(v=>{const box=v.getBoundingClientRect(),container=v.parentElement.getBoundingClientRect();return{width:box.width,height:box.height,containerWidth:container.width,containerHeight:container.height,fit:getComputedStyle(v).objectFit}});assert.ok(Math.abs(fitted.width-fitted.containerWidth)<1&&Math.abs(fitted.height-fitted.containerHeight)<1,'Video must fit the oval rather than render at its intrinsic resolution');assert.equal(fitted.fit,'cover');await page.screenshot({path:'output/playwright/kyc-v12-clean-oval-'+method+'.png'});const paused=await page.evaluate(()=>window.__captures);await page.waitForTimeout(1000);assert.equal(await page.evaluate(()=>window.__captures),paused,'Neutral face must pause at smile stage');assert.equal(await page.evaluate(()=>window.__published),0)
  ...
  await page.evaluate(()=>{window.__face=false;window.__smile=true});await page.waitForTimeout(650);assert.equal(await page.evaluate(()=>window.__captures),stopped,'No face cannot progress');await page.evaluate(()=>window.__face=true);await page.locator('#capture-test .confirm').waitFor({state:'visible'});assert.equal(await page.evaluate(()=>window.__published),0);await page.locator('#capture-test .confirm').click();await page.waitForFunction(()=>window.__published===1)
  assert.equal(await page.evaluate(()=>window.__constraintCalls),0);assert.equal(await page.evaluate(()=>window.__imageCount),8);assert.equal(submissions,0)
  await page.screenshot({path:'output/playwright/kyc-v12-'+method+'.png'});console.log('PASS '+method+': neutral pauses, smile resumes, stopped smile/no face pause, single completion; phone layouts; zero uploads (synthetic landmarks).');await page.close()
 }
