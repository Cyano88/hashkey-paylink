...
try {
 for (const method of ['biometric-kyc','doc-verification']) {
  ...
  await page.evaluate(()=>{const main=document.querySelector('main');main.hidden=false;for(const child of main.children)child.hidden=true;const wrapper=document.createElement('selfie-capture-wrapper');wrapper.id='capture-test';wrapper.setAttribute('theme-color','#171717');wrapper.setAttribute('show-navigation','true');main.append(wrapper)})
  const button=page.locator('#capture-test button.btn-primary');await button.waitFor({timeout:12000});await page.waitForFunction(()=>{const r=document.getElementById('capture-test')?.shadowRoot;return r&&[...r.querySelectorAll('button')].some(b=>b.classList.contains('btn-primary')&&!b.disabled)})
  ...
  await page.screenshot({path:'output/playwright/kyc-v11-'+method+'.png'});console.log('PASS '+method+': neutral pauses, smile resumes, stopped smile/no face pause, single completion; phone layouts; zero uploads (synthetic landmarks).');await page.close()
 }
