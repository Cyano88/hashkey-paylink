...
assert.equal(review.body.brand.merchantName, policy.merchantName)
assert.equal(review.body.brand.brandImageUrl, policy.brandImageUrl ?? null)
...
assert.match(payerPageSource, /!isActive\s*&&\s*!creatorFundingBlocked\s*&&\s*\(/)
...
assert.match(checkoutChromeSource, /src="\/hash-logo-transparent\.png"/)
assert.match(checkoutChromeSource, /h-3\.5 w-3\.5 object-contain dark:invert/)
