...
assert.match(appSource, /<Route index element={<StreamPayLanding\s*\/>}/)
assert.match(appSource, /path="docs" element={<StreamPayDocs\s*\/>}/)
assert.match(appSource, /path="agreements" element={<AgreementDashboard\s*\/>}/)
...
assert.match(formSource, /const AGREEMENTS_API = '\/api\/hashpaystream\/v2\/agreements'/)
assert.match(formSource, /fetch\(AGREEMENTS_API/)
assert.doesNotMatch(formSource, /\/api\/hashpaystream\/arc-agreements/)
