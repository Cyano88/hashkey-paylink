...
const handler=createStockWalletBalancesHandler({now:()=>now,policy:async()=>authorized?{partnerId:'project_a',environment:'live',checkoutMode:'human'}:null,balances:async(owner,wallet)=>{seen={owner,wallet};return {holdings:[{asset:{address:asset,symbol:'TESTx',name:'Test',icon:''},units:125n,decimals:2}],cash:0n,gas:0n,complete,observedAt:stale?now-61000:now,blockNumber:123n,blockHash:'0x01',fullScanAt:now}},prices:async()=>priced?{[asset]:{usd:10,fetchedAt:now,change:null,volume:0}}:{}})
