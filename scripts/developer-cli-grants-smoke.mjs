...
await authCommand('auth logout', {}, { fetcher, sessionStore })
const stockLogin=await authCommand('auth login',{project:projectId,scopes:'project:read,xstocks-agreement:read,xstocks-agreement:create,keys:manage'},{fetcher,sessionStore})
assert.equal((await call({action:'approve',id:session.grant.id,userCode:stockLogin.userCode},'Bearer owner')).statusCode,200)
assert.ok(await resolveCliGrant(session.token,'xstocks-agreement:read',deps))
assert.ok(await resolveCliGrant(session.token,'xstocks-agreement:create',deps))
assert.equal(await resolveCliGrant(session.token,'agreement:create',deps),null)
assert.equal(await resolveCliGrant(session.token,'checkout:create',deps),null)
console.log('Owner-approved xStocks grant is isolated from Arc Agreement and checkout scopes.')
