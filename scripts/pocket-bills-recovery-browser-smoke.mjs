...
try {
 ...
 assert.equal(await page.evaluate(()=>window.checks),1,'Pending retry must not run early')
 await page.clock.runFor(1_100)
 await page.waitForFunction(()=>window.checks===2)
 assert.notEqual(await page.evaluate(()=>window.bills.status),'successful','Unconfirmed payment must not become successful')
