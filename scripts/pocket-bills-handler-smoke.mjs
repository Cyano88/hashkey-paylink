...
assert.equal(quoted.body.data.intent.billAmountUsdc, '0.071429')
assert.equal(quoted.body.data.intent.platformFeeUsdc, '0.000178')
assert.equal(quoted.body.data.intent.amountUsdc, '0.071607')
...
assert.equal(quoteReplay.body.data.intent.amountUsdc, '0.071607', 'Replays must not add the fee twice')
...
assert.equal(lastVerificationInput.minAmount, '0.071607', 'On-chain payment must include the platform fee')
