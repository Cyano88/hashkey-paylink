...
try{for(const mode of ['accepted','confirmed','timeout','reverted']){
 ...
 const result=await page.evaluate(()=>({status:window.send.status,journal:!!localStorage.getItem('pocket:evm-send:operation:v1'),confirmation:window.confirmRequested}));assert.equal(result.confirmation,true);
 assert.equal(result.status,mode==='confirmed'?'successful':mode==='reverted'?'idle':'submitted');assert.equal(result.journal,mode==='accepted'||mode==='timeout');
 if(mode==='accepted'){await page.evaluate(()=>window.confirmTransfer({state:'confirmed',txHash:'0x'+'a'.repeat(64)}));await page.waitForFunction(()=>window.send.status==='successful');assert.equal(await page.evaluate(()=>localStorage.getItem('pocket:evm-send:operation:v1')),null)}
 await page.close()
}console.log('PASS: acceptance and uncertain transport remain Processing with journal; confirmation alone succeeds; explicit on-chain failure clears the attempt')
