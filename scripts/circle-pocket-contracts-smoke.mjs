...
assert.match(pocketRouteShellSource, /completedDistance < 68/)
assert.match(pocketRouteShellSource, /pullFrame\.current = window\.requestAnimationFrame/)
...
assert.match(paymentPageSource, /\{ value: 'solana', label: 'Solana Soon', disabled: true \}/)
assert.match(paymentPageSource, /isHostedCheckout && \(\s*<div[^>]*>[\s\S]*?Payment network[\s\S]*?<PocketSelect/)
assert.match(paymentPageSource, /!isHostedCheckout && <div className="flex justify-center px-4 pb-0 pt-4">/)
...
assert.match(paymentPageSource, /c === 'arc' && <span className="text-\[8px\].*?>Test<\/span>/)
