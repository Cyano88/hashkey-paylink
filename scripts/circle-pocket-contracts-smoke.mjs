...
const routeCases = [
  ['/home', { section: 'home', view: 'overview' }],
  ['/profile', { section: 'profile', view: 'details' }],
  ...
  ['/activity/purchases', { section: 'activity', view: 'purchases' }],
...
assert.equal((await readFxQuote('1.309814')).stale, true)
...
const profileSaveEnvelope = {
  ...
  status: 'completed',
  data: {
    profile: { ...pocketProfile, pocketId: '34567890', updatedAt: '2026-07-15T00:00:01.000Z' },
    unchanged: false,
  },
}
...
assert.doesNotMatch(pocketReceiveMethodSource, /Receive with email/)
...
const pocketRequestPageSource = await readFile(new URL('../src/pocket/pages/PocketMoveUsdcPage.tsx', import.meta.url), 'utf8')
assert.doesNotMatch(pocketRequestPageSource, /PocketReceiveMethodPanel|PocketEmailWalletDetails|Pocket wallet/)
...
assert.match(pocketFxEndpointSource, /PAYCREST_LAST_KNOWN_MAX_AGE_MS/)
assert.match(pocketFxEndpointSource, /readDurableJson/)
...
assert.match(pocketActivityHookSource, /pocketActivityCache\.set\(cacheKey, nextRows\)/)
assert.match(pocketActivityHookSource, /initialController\.abort\(\), 2_800/)
assert.match(pocketActivityHookSource, /if \(!hasSnapshot\) \{[\s\S]*recent: true/)
assert.match(pocketActivityHookSource, /Promise\.race\(\[getAccessToken\(\), initialDeadline\]\)/)
assert.match(pocketActivityHookSource, /POCKET_ACTIVITY_CACHE_TTL_MS = 7 \* 24 \* 60 \* 60_000/)
assert.match(pocketActivityHookSource, /writeDurableActivity\(email, cacheRecent, nextRows\)/)
assert.match(pocketActivityHookSource, /const deadlineExceeded = reason instanceof DOMException/)
assert.match(pocketActivityHookSource, /readPocketActivity\(\{ accessToken: retryToken, recent: true \}\)/)
assert.doesNotMatch(pocketActivityHookSource, /Activity did not respond within 3 seconds/)
...
assert.match(pocketProfileHookSource, /pocketId: draft\.pocketId/)
assert.doesNotMatch(pocketProfileHookSource, /firstName: draft\.firstName|lastName: draft\.lastName|email: email \|\| draft\.email/)
...
const pocketSecureWalletSessionSource = await readFile(new URL('../src/pocket/lib/pocketSecureWalletSession.ts', import.meta.url), 'utf8')
const pocketPaymentBiometricsSource = await readFile(new URL('../src/pocket/lib/pocketPaymentBiometrics.ts', import.meta.url), 'utf8')
const pocketPaymentSecurityGateSource = await readFile(new URL('../src/pocket/components/PocketPaymentSecurityGate.tsx', import.meta.url), 'utf8')
...
// Wallet readiness must propagate failed links; behavioral coverage lives in pocket-wallet-readiness-smoke.mjs.
assert.doesNotMatch(pocketWalletControllerSource, /\.catch\(\(\) => null\)/)
assert.match(pocketWalletControllerSource, /updatedAt: linked\?\.link\?\.updatedAt/)
assert.match(pocketWalletControllerSource, /currentSession\.wallet\.address\.toLowerCase\(\) === walletAddress\.toLowerCase\(\)/)
assert.match(pocketWalletControllerSource, /sharedPendingEvmSessions\.get\(key\)/)
assert.match(pocketWalletControllerSource, /activePocketEvmSession\(email, network, walletAddress\)/)
assert.match(pocketWalletControllerSource, /if \(options\.allowSharedSession\)/)
assert.match(pocketWalletControllerSource, /options: \{ allowSharedSession\?: boolean \} = \{ allowSharedSession: true \}/)
assert.match(pocketWalletControllerSource, /export async function unlockPocketBaseWallet/)
assert.match(pocketWalletControllerSource, /const sharedPocketUnlocks = new Map/)
assert.match(pocketWalletControllerSource, /export async function restorePocketWalletSession/)
...
assert.doesNotMatch(pocketWalletControllerSource, /isExpiredEmailChallenge|connectCircleEvmEmailWallet\(email, network\)[\s\S]*connectCircleEvmEmailWallet\(email, network\)/)
assert.match(pocketSecureWalletSessionSource, /AccessControl\.NONE/)
assert.match(pocketSecureWalletSessionSource, /NativeBiometric\.getCredentials\(\{ server: key \}\)/)
assert.doesNotMatch(pocketSecureWalletSessionSource, /NativeBiometric\.getSecureCredentials\(\{ server: key \}\)/)
assert.match(pocketSecureWalletSessionSource, /export async function pocketSecureWalletSessionAvailable/)
assert.match(pocketSecureWalletSessionSource, /refreshCircleEvmEmailSession/)
assert.match(pocketSecureWalletSessionSource, /savePocketSecureWalletSession/)
assert.match(pocketSecureWalletSessionSource, /const sessionOperations = new Map/)
assert.match(pocketSecureWalletSessionSource, /withSessionLock/)
assert.match(pocketSecureWalletSessionSource, /PocketWalletSessionRecoveryRequiredError/)
assert.doesNotMatch(pocketSecureWalletSessionSource, /Pocket could not refresh the secure wallet session/)
assert.match(pocketSecureWalletSessionSource, /SESSION_REFRESH_INTERVAL_MS = 12 \* 60 \* 60 \* 1000/)
assert.match(pocketSecureWalletSessionSource, /JSON\.stringify\(\{ session, savedAt: Date\.now\(\) \}/)
assert.match(pocketSecureWalletSessionSource, /if \(stored\) return stored/)
assert.match(pocketPaymentBiometricsSource, /AccessControl\.BIOMETRY_CURRENT_SET/)
assert.match(pocketPaymentBiometricsSource, /export function pocketPaymentBiometricsConfigured/)
assert.match(pocketPaymentBiometricsSource, /negativeButtonText: 'Use Pocket PIN'/)
assert.match(pocketPaymentSecurityGateSource, /Create your Pocket PIN/)
assert.match(pocketPaymentSecurityGateSource, /verifyPocketPaymentPin/)
assert.match(pocketPaymentSecurityGateSource, /available && !pocketPaymentBiometricsConfigured\(\) \? 'offer' : 'ready'/)
assert.match(pocketPaymentSecurityGateSource, /if \(!setupPin\) await verifyPocketPaymentPin\(getAccessToken, value\)/)
assert.match(pocketPaymentSecurityGateSource, /Confirm your Pocket PIN once/)
assert.match(pocketPaymentSecurityGateSource, /PAYMENT_SECURITY_CONFIGURED_PREFIX/)
assert.match(pocketPaymentSecurityGateSource, /initialSecurityState\(email\)/)
assert.match(pocketPaymentSecurityGateSource, /configured && !resetPending \? 'ready' : 'loading'/)
assert.match(pocketPaymentSecurityGateSource, /configuredOnThisDevice && !reset/)
assert.match(pocketWalletControllerSource, /readPocketSecureWalletSession/)
assert.match(pocketWalletControllerSource, /savePocketSecureWalletSession/)
assert.doesNotMatch(pocketWalletControllerSource, /savePocketSecureWalletSession\(email, session\)\.catch/)
assert.match(pocketWalletControllerSource, /resumeCircleSolanaEmailWallet/)
assert.match(pocketSecureWalletSessionSource, /productionEvmTopology\?\.wallets\?\.\[network\]/)
assert.match(pocketWalletControllerSource, /evmSessionRef\.current = session/)
assert.doesNotMatch(pocketSecureWalletSessionSource, /console\.|sessionStorage/)
...
assert.match(pocketWithdrawalControllerSource, /validatePocketWithdrawal\(\{ network, address, amount, balance: options\?\.balanceOverride \?\? balance \}\)/)
assert.ok(pocketWithdrawalControllerSource.indexOf('validatePocketWithdrawal({ network, address, amount, balance })') < pocketWithdrawalControllerSource.indexOf('options?.walletOverride ?? wallet ?? await ensureWallet(network)'))
assert.match(pocketWithdrawalControllerSource, /options\?\.walletOverride \?\? wallet \?\? await ensureWallet\(network\)/)
assert.match(pocketWithdrawalControllerSource, /sendCircleSolanaTransfer\(\{/)
assert.match(pocketWithdrawalControllerSource, /reconcileCircleSolanaTransfer\(\{/)
assert.match(pocketWithdrawalControllerSource, /onAccepted: identifiers =>/)
assert.match(pocketWithdrawalControllerSource, /state: 'accepted'/)
assert.match(pocketWithdrawalControllerSource, /handedOff = Boolean\(result\.txHash\) \|\| circleAccepted/)
assert.match(pocketWithdrawalControllerSource, /clearSolanaOperation\(\)/)
assert.match(pocketWithdrawalControllerSource, /registerPocketPaymentPreparer\(prepare\)/)
assert.doesNotMatch(pocketWithdrawalControllerSource, /Your send was accepted\. Pocket is confirming delivery\./)
assert.doesNotMatch(pocketWithdrawalControllerSource, /Solana send confirmed/)
assert.match(pocketWithdrawalControllerSource, /session,/)
assert.match(pocketWithdrawalControllerSource, /recipient,/)
assert.match(pocketWithdrawalControllerSource, /amount: amount\.trim\(\)/)
assert.match(pocketWithdrawalControllerSource, /executePocketEvmTransfer\(\{/)
assert.match(pocketWithdrawalControllerSource, /confirm: false/)
assert.match(pocketWithdrawalControllerSource, /setPending\(false\)/)
assert.match(pocketWithdrawalControllerSource, /USDC sent on \$\{networkLabel\}/)
assert.match(pocketWithdrawalControllerSource, /if \(operation\.state === 'accepted'\) \{\s*clearSolanaOperation\(\)\s*return/)
assert.match(pocketWithdrawalControllerSource, /if \(operation\.state === 'accepted'\) \{\s*clearEvmOperation\(\)\s*return/)
assert.match(pocketWithdrawalControllerSource, /savedOperation\?\.state === 'preparing' \|\| savedOperation\?\.state === 'submitted'/)
assert.match(pocketWithdrawalControllerSource, /if \(handedOff\) \{\s*if \(network === 'solana'\) clearSolanaOperation\(\)/)
assert.doesNotMatch(pocketWithdrawalControllerSource, /fetch\(|['"]\/api\/|\/api\/solana-build-tx|\/api\/solana-relay|preparePocketSolanaTransfer|submitPocketSolanaTransfer|signCircleSolanaTransaction/)
const layoutSource = await readFile(new URL('../src/Layout.tsx', import.meta.url), 'utf8')
const pocketNativeBridgeSource = await readFile(new URL('../src/pocket/components/PocketNativeBridge.tsx', import.meta.url), 'utf8')
const pocketAssistantPageSource = await readFile(new URL('../src/pocket/pages/PocketAssistantPage.tsx', import.meta.url), 'utf8')
const appSource = await readFile(new URL('../src/App.tsx', import.meta.url), 'utf8')
const mainSource = await readFile(new URL('../src/main.tsx', import.meta.url), 'utf8')
const pocketHostAppSource = await readFile(new URL('../src/pocket/PocketHostApp.tsx', import.meta.url), 'utf8')
const pocketLegalDocumentSource = await readFile(new URL('../src/pocket/pages/PocketLegalDocumentPage.tsx', import.meta.url), 'utf8')
const termsDocsSource = await readFile(new URL('../src/pages/docs/TermsDocs.tsx', import.meta.url), 'utf8')
const privacyDocsSource = await readFile(new URL('../src/pages/docs/PrivacyDocs.tsx', import.meta.url), 'utf8')
const pocketThemeToggleSource = await readFile(new URL('../src/pocket/components/PocketThemeToggle.tsx', import.meta.url), 'utf8')
const pocketLandingSource = await readFile(new URL('../src/pocket/pages/PocketLandingPage.tsx', import.meta.url), 'utf8')
const pocketLoadingStateSource = await readFile(new URL('../src/pocket/components/PocketLoadingState.tsx', import.meta.url), 'utf8')
const pocketSessionSplashSource = await readFile(new URL('../src/pocket/hooks/usePocketSessionSplash.ts', import.meta.url), 'utf8')
const pocketSessionSplashViewSource = await readFile(new URL('../src/pocket/components/PocketSessionSplash.tsx', import.meta.url), 'utf8')
const pocketAuthBrandSource = await readFile(new URL('../src/pocket/components/PocketAuthBrand.tsx', import.meta.url), 'utf8')
const pocketProgressDotsSource = await readFile(new URL('../src/pocket/components/PocketProgressDots.tsx', import.meta.url), 'utf8')
const pocketLightSurfaceSource = await readFile(new URL('../src/pocket/hooks/usePocketLightSurface.ts', import.meta.url), 'utf8')
const themeContextSource = await readFile(new URL('../src/lib/ThemeContext.tsx', import.meta.url), 'utf8')
const pocketBottomNavSource = await readFile(new URL('../src/pocket/components/PocketBottomNav.tsx', import.meta.url), 'utf8')
...
const capacitorConfigSource = await readFile(new URL('../capacitor.config.ts', import.meta.url), 'utf8')
const mainActivitySource = await readFile(new URL('../android/app/src/main/java/com/hashpaylink/pocket/MainActivity.java', import.meta.url), 'utf8')
const solanaContextSource = await readFile(new URL('../src/lib/SolanaContext.tsx', import.meta.url), 'utf8')
...
assert.doesNotMatch(layoutSource, /onWalletChange/)
...
assert.match(mainSource, /isPocketHostname\(window\.location\.hostname\)/)
assert.doesNotMatch(mainSource, /SplashScreen\.hide\(\)/)
assert.match(mainSource, /import\('\.\/pocket\/PocketHostApp'\)/)
assert.match(mainSource, /import\('\.\/App'\)/)
assert.doesNotMatch(mainSource, /import App from '\.\/App'/)
assert.match(mainSource, /if \(isPocketNativeRuntime\(\)\) \{[\s\S]*aria-label="Opening Pocket"[\s\S]*bg-\[#F5F5F7\]/)
assert.doesNotMatch(mainSource, /aria-label="Opening Pocket"[\s\S]{0,300}c-purse\.svg/)
assert.match(pocketHostAppSource, /<Route path="\*" element=\{<CirclePocketApp \/>\} \/>/)
assert.match(pocketHostAppSource, /lazy\(\(\) => import\('\.\.\/pages\/X402Receipt'\)\)/)
assert.match(pocketHostAppSource, /<Route path="docs\/terms"[\s\S]*PocketLegalDocumentPage document="terms"/)
assert.match(pocketHostAppSource, /<Route path="docs\/privacy"[\s\S]*PocketLegalDocumentPage document="privacy"/)
assert.match(pocketHostAppSource, /<Route path="docs\/account-deletion"[\s\S]*PocketLegalDocumentPage document="account-deletion"/)
assert.ok(pocketHostAppSource.indexOf('path="docs/terms"') < pocketHostAppSource.indexOf('<Route element={<Layout />}>'))
assert.match(pocketLegalDocumentSource, /document === 'terms' \? <TermsDocs \/> : document === 'privacy' \? <PrivacyDocs \/> : <AccountDeletionDocs \/>/)
assert.doesNotMatch(pocketLegalDocumentSource, /PocketTopSwitch|PocketThemeToggle/)
assert.match(pocketLegalDocumentSource, /fixed inset-0 flex h-\[100dvh\] min-h-0 flex-col overflow-hidden/)
assert.match(pocketLegalDocumentSource, /data-pocket-legal-scroll/)
assert.match(pocketLegalDocumentSource, /min-h-0 flex-1 overflow-x-hidden overflow-y-auto/)
assert.match(pocketLegalDocumentSource, /overscroll-y-contain/)
assert.match(appSource, /window\.location\.replace\(destination\)/)
assert.ok(appSource.indexOf('if (IS_POCKET_HOST)') < appSource.indexOf('if (isStreamPayRoute)'))
assert.match(appSource, /window\.location\.replace\('https:\/\/hashpaystream\.app\/docs'\)/)
assert.doesNotMatch(appSource, /hostname === 'hashpaystream\.app'/)
assert.doesNotMatch(createLinkSource, /title: 'Hash Paystream'/)
assert.match(layoutSource, /<CPurseIcon size=\{32\}/)
assert.match(layoutSource, /classList\.toggle\('dark', theme === 'dark'\)/)
assert.doesNotMatch(layoutSource, /isPocketAppPage \? false : theme === 'dark'/)
assert.doesNotMatch(layoutSource, /<PocketThemeToggle \/>/)
assert.match(layoutSource, /<PocketNotificationButton \/>/)
assert.doesNotMatch(layoutSource, /<PocketAccountMenu \/>/)
assert.match(layoutSource, /--pocket-status-bar-inset/)
assert.match(pocketNativeBridgeSource, /StatusBar\.getInfo\(\)/)
assert.match(pocketNativeBridgeSource, /--pocket-status-bar-inset/)
assert.match(pocketNativeBridgeSource, /nativeInsetCssPixels/)
assert.match(pocketNativeBridgeSource, /info\.topPx, info\.density/)
assert.doesNotMatch(pocketNativeBridgeSource, /setOverlaysWebView/)
assert.match(pocketNativeBridgeSource, /data-pocket-light-surface/)
assert.match(pocketNativeBridgeSource, /dataset\.pocketLightSurface === 'true'/)
assert.match(pocketAssistantPageSource, /--pocket-status-bar-inset/)
assert.match(layoutSource, /pocketRoute\?\.section === 'bills'.*pocketRoute\?\.section === 'activity'/)
for (const weight of [400, 500, 600, 700, 800]) assert.ok(mainSource.includes("import '@fontsource/plus-jakarta-sans/" + weight + ".css'"))
assert.match(mainSource, /loginMethods: \['email'\]/)
assert.match(mainSource, /disableAllExternalWallets: true/)
assert.match(mainSource, /theme: theme === 'dark' \? 'dark' : 'light'/)
assert.doesNotMatch(mainSource, /isPocketSurface \? 'light'/)
assert.doesNotMatch(mainSource, /loginMethodsAndOrder|loginMethods: \['email', 'wallet'\]/)
assert.match(pocketThemeToggleSource, /data-pocket-theme-toggle/)
assert.match(pocketThemeToggleSource, /Switch Pocket to light mode/)
assert.match(pocketThemeToggleSource, /Switch Pocket to dark mode/)
assert.doesNotMatch(pocketLandingSource, /PocketThemeToggle/)
assert.doesNotMatch(pocketLandingSource, /Guest Login|connected wallet/)
assert.doesNotMatch(pocketLandingSource, /Secured by Privy|privy-mark-logo\.png/)
assert.match(pocketLandingSource, /splashState === 'launching'/)
assert.match(pocketLandingSource, /getBoundingClientRect\(\)/)
assert.match(pocketLandingSource, /duration-\[820ms\] ease-\[cubic-bezier\(0\.16,1,0\.3,1\)\]/)
assert.match(pocketLandingSource, /splashLogoVisible \? 'opacity-100' : 'opacity-0'/)
assert.match(pocketLandingSource, /motion-reduce:hidden/)
assert.doesNotMatch(pocketLandingSource, /pointer-events-auto|<nav/)
assert.match(pocketSessionSplashSource, /pocket_splash_shown/)
assert.match(pocketSessionSplashSource, /failSafeTimer/)
assert.match(pocketSessionSplashSource, /prefers-reduced-motion: reduce/)
assert.match(pocketSessionSplashSource, /dataset\.pocketRuntime === 'native'/)
assert.match(pocketSessionSplashSource, /if \(nativeRuntime\) return 'entering'/)
assert.match(pocketSessionSplashSource, /setState\('mark'\), 70/)
assert.match(pocketSessionSplashSource, /setState\('assembling'\), 260/)
assert.match(pocketSessionSplashSource, /setState\('holding'\), 280/)
assert.match(pocketSessionSplashSource, /state !== 'holding'/)
assert.match(pocketSessionSplashSource, /setState\('launching'\), 12_000/)
assert.match(pocketSessionSplashSource, /setState\('idle'\), 320/)
assert.match(pocketSessionSplashViewSource, /By Hash PayLink/)
assert.match(pocketSessionSplashViewSource, /<CPurseIcon/)
assert.match(pocketSessionSplashViewSource, /launchBridge\?\.ready\(\)/)
assert.match(pocketSessionSplashViewSource, /usePocketLightSurface\(state !== 'idle'\)/)
assert.doesNotMatch(pocketSessionSplashViewSource, /CPurseHeroMark/)
assert.doesNotMatch(pocketSessionSplashViewSource, /Taking longer than expected/)
assert.match(pocketLoadingStateSource, /aria-busy="true"/)
assert.match(pocketLoadingStateSource, /motion-reduce:animate-none/)
assert.match(pocketLoadingStateSource, /navigationDisabled/)
assert.doesNotMatch(pocketLoadingStateSource, /Restoring your Pocket|USDC/)
assert.match(indexHtmlSource, /<script src="\/theme-init\.js"><\/script>/)
assert.match(indexHtmlSource, /location\.hostname === 'app\.hashpaylink\.com' && \/\\bwv\\b\/i\.test\(navigator\.userAgent\)/)
assert.match(indexHtmlSource, /id="pocket-native-boot" aria-label="Opening Pocket"/)
assert.doesNotMatch(indexHtmlSource, /id="pocket-native-boot"[\s\S]{0,240}c-purse\.svg/)
assert.match(capacitorConfigSource, /launchShowDuration: 0/)
assert.ok(mainActivitySource.includes('launchSplash.setKeepOnScreenCondition(() -> keepLaunchSplash)'))
assert.match(mainActivitySource, /addJavascriptInterface\(new PocketLaunchBridge\(\), "PocketLaunch"\)/)
assert.match(mainActivitySource, /public void ready\(\)[\s\S]*keepLaunchSplash = false/)
assert.ok(mainActivitySource.indexOf('SplashScreen.installSplashScreen(this)') < mainActivitySource.indexOf('super.onCreate(savedInstanceState)'))
assert.match(themeInitSource, /classList\.toggle\('dark', dark\)/)
assert.match(themeInitSource, /style\.colorScheme = dark \? 'dark' : 'light'/)
assert.doesNotMatch(indexHtmlSource, /fonts\.googleapis|fonts\.gstatic/)
assert.match(solanaContextSource, /login\(\{ loginMethods: \['email'\] \}\)/)
assert.doesNotMatch(solanaContextSource, /useConnectWallet|connectWallet\(|walletChainType|walletList/)
...
for (const { path, source } of standalonePocketSources) {
  const isExtractedAgentHashPage = path.endsWith('/pages/PocketAssistantPage.tsx')
  assert.doesNotMatch(
    source,
    isExtractedAgentHashPage
      ? /CreateLink|AgentWorkspace|openCirclePocketMode|circlePocketMode|circlePocketShellActive/
  ...
  assert.doesNotMatch(source, /loginMethods: \['email', 'wallet'\]/, `${path} must keep Privy identity email-only`)
  assert.doesNotMatch(source, /\b(?:RefreshCw|RotateCw)\b/, `${path} must not render generic refresh glyphs`)
}
assert.match(circlePocketAppSource, /const PocketBillsPage = lazy\(\(\) => import\('\.\/pages\/PocketBillsPage'\)\)/)
assert.match(circlePocketAppSource, /route\?\.section === 'bills'/)
assert.match(circlePocketAppSource, /<PocketBillsPage view=\{route\.view\} \/>/)
assert.match(circlePocketAppSource, /const PocketActivityPage = lazy\(\(\) => import\('\.\/pages\/PocketActivityPage'\)\)/)
...
assert.match(circlePocketAppSource, /const PocketAssistantPage = lazy\(\(\) => import\('\.\/pages\/PocketAssistantPage'\)\)/)
assert.match(circlePocketAppSource, /route\?\.section === 'assistant'/)
assert.match(circlePocketAppSource, /<PocketAssistantPage \/>/)
assert.match(circlePocketAppSource, /const PocketHomePage = lazy\(\(\) => import\('\.\/pages\/PocketHomePage'\)\)/)
assert.match(circlePocketAppSource, /route\?\.section === 'home'/)
assert.match(circlePocketAppSource, /<PocketHomePage \/>/)
assert.match(circlePocketAppSource, /const PocketProfilePage = lazy\(\(\) => import\('\.\/pages\/PocketProfilePage'\)\)/)
assert.match(circlePocketAppSource, /route\?\.section === 'profile'/)
assert.doesNotMatch(circlePocketAppSource, /PocketX402Page/)
assert.match(circlePocketAppSource, /const PocketLandingPage = lazy\(\(\) => import\('\.\/pages\/PocketLandingPage'\)\)/)
assert.match(circlePocketAppSource, /walletUnlockState.*'reconnect'.*'ready'/)
assert.match(circlePocketAppSource, /reconnectPocketBaseWallet\(\{ authenticated, email, getAccessToken \}\)/)
assert.match(circlePocketAppSource, /else if \(!authenticated\) content = <PocketPageBoundary active='home'><PocketLandingPage/)
assert.match(circlePocketAppSource, /walletUnlockState === 'reconnect' \|\| \(walletUnlockState === 'ready' && paymentSecurityReady\)/)
assert.match(circlePocketAppSource, /usePocketSessionSplash\(launchSurface, launchDestinationReady\)/)
assert.match(circlePocketAppSource, /onInitialStateResolved=\{\(\) => setPaymentSecurityReady\(true\)\}/)
assert.match(pocketPaymentSecurityGateSource, /if \(state !== 'loading'\) onInitialStateResolved\?\.\(\)/)
assert.doesNotMatch(circlePocketAppSource, /sessionResolved[\s\S]{0,180}!profile\.loadError/)
assert.match(circlePocketAppSource, /else if \(landing\) content = <PocketPageBoundary active="home"><PocketLandingPage \/><\/PocketPageBoundary>/)
assert.match(circlePocketAppSource, /const PocketMoveUsdcPage = lazy\(\(\) => import\('\.\/pages\/PocketMoveUsdcPage'\)\)/)
...
assert.match(circlePocketAppSource, /const PocketMoveBankPage = lazy\(\(\) => import\('\.\/pages\/PocketMoveBankPage'\)\)/)
...
assert.match(circlePocketAppSource, /const PocketMovePosPage = lazy\(\(\) => import\('\.\/pages\/PocketMovePosPage'\)\)/)
...
const pocketBillsPanelSource = await readFile(new URL('../src/pocket/features/bills/PocketBillsPanel.tsx', import.meta.url), 'utf8')
const pocketPaymentLiquidityControllerSource = await readFile(new URL('../src/pocket/controllers/usePocketPaymentLiquidityController.ts', import.meta.url), 'utf8')
const pocketMobileNumberInputSource = await readFile(new URL('../src/pocket/features/bills/PocketMobileNumberInput.tsx', import.meta.url), 'utf8')
assert.match(pocketBillsPageSource, /usePocketIdentity\(\)/)
assert.match(pocketBillsPageSource, /usePocketWallets\(\{ authenticated, email, getAccessToken \}\)/)
assert.match(pocketBillsPageSource, /usePocketWalletController\(\{ authenticated, email, getAccessToken, onWalletReady \}\)/)
assert.match(pocketBillsPageSource, /usePocketBillsController\(\{/)
assert.match(pocketBillsPageSource, /usePocketPaymentLiquidityController\(\{/)
assert.ok(
  pocketBillsPageSource.indexOf('await paymentLiquidity.ensureLiquidity()') < pocketBillsPageSource.indexOf('await bills.pay()'),
  'Bills must confirm destination liquidity before starting the existing bill payment.',
)
assert.match(pocketBillsPageSource, /<PocketBillsPanel/)
assert.match(pocketBillsPageSource, /<PocketRouteShell active="bills"/)
assert.match(pocketBillsPageSource, /import\.meta\.env\.DEV/)
assert.match(pocketBillsPageSource, /\['localhost', '127\.0\.0\.1', '::1'\]\.includes\(window\.location\.hostname\)/)
assert.match(pocketBillsPageSource, /new URLSearchParams\(location\.search\)\.get\('preview'\) === '1'/)
...
assert.match(pocketBillsPanelSource, /<PocketMobileNumberInput/)
assert.match(pocketBillsPanelSource, /onChange=\{\(\{ phoneNumber, networkId \}\) =>/)
assert.match(pocketBillsPanelSource, /Test mode · USDC payment is real · no live service is delivered/)
assert.match(pocketBillsPanelSource, /disabled=\{locked \|\| \(!preview && bills\.environment === 'sandbox'\)\}/)
assert.match(pocketBillsPanelSource, /filter\(service => view !== 'data' \|\| \/\^\(mtn\|airtel\|glo\|etisalat\)-data\$\//)
assert.match(pocketBillsPanelSource, /label: view === 'data' \? dataServiceLabel\(service\.name\) : service\.name/)
assert.match(pocketBillsPanelSource, /Insufficient across networks/)
assert.match(pocketBillsPanelSource, /Moving USDC/)
assert.match(pocketBillsPanelSource, /import PocketPaymentSuccess from/)
assert.match(pocketBillsPanelSource, /<PocketPaymentSuccess receipt=\{billReceipt\} title=\{billName\} onDone=\{bills\.resetResult\}/)
assert.doesNotMatch(pocketBillsPanelSource, /<UnifiedReceipt receipt=\{billReceipt\}/)
assert.match(pocketBillsPanelSource, /brandName: 'Pocket'/)
assert.match(pocketBillsPanelSource, /brandKind: 'pocket'/)
assert.match(pocketPaymentLiquidityControllerSource, /const NETWORKS: PocketCheckoutNetwork\[\] = \['base', 'arbitrum', 'solana'\]/)
assert.doesNotMatch(pocketPaymentLiquidityControllerSource, /\['base', 'arbitrum', 'arc', 'solana'\]/)
...
assert.match(pocketPaymentLiquidityControllerSource, /\(checkpoint\?\.phase === 'submitted' \|\| checkpoint\?\.phase === 'completed'\) && checkpoint\.txHash/)
assert.match(pocketPaymentLiquidityControllerSource, /if \(checkpoint\?\.phase === 'completed'\) \{[\s\S]*?setStatus\('arrived'\)[\s\S]*?return destinationWallet/)
...
assert.match(pocketPaymentLiquidityControllerSource, /let activeCheckpoint = checkpoint\?\.phase === 'completed' \? null : checkpoint/)
...
assert.match(pocketPaymentLiquidityControllerSource, /USDC move submitted\. Pocket will continue automatically\./)
...
assert.match(pocketMobileNumberInputSource, /manualNetworkOverride\.current = true/)
assert.match(pocketMobileNumberInputSource, /detectNigerianMobileNetwork\(nextPhone\)/)
assert.match(pocketMobileNumberInputSource, /Network detected\. You can change it for a ported number\./)
for (const mark of ['mtn.svg', 'airtel.svg', 'glo.svg', '9mobile.svg']) {
  assert.match(pocketMobileNumberInputSource, new RegExp(mark.replace('.', '\\.')))
}
const pocketSelectSource = await readFile(new URL('../src/pocket/components/PocketSelect.tsx', import.meta.url), 'utf8')
assert.match(pocketSelectSource, /const unavailable = disabled \|\| options\.length === 0/)
assert.match(pocketSelectSource, /\{open && options\.length > 0 && \(/)
...
assert.match(pocketRouteShellSource, /<PocketBottomNav active=\{active\} disabled=\{navigationDisabled\} keyboardOpen=\{keyboardOpen\}/)
assert.match(pocketBottomNavSource, /disabled=\{disabled\}/)
assert.match(pocketBottomNavSource, /aria-disabled=\{disabled \|\| undefined\}/)
assert.match(pocketRouteShellSource, /onTouchStart=.*startPull/)
assert.match(pocketRouteShellSource, /onTouchEnd=.*finishPull/)
assert.match(pocketRouteShellSource, /Promise\.race\(\[refreshWork, new Promise\(resolve => window\.setTimeout\(resolve, 3_000\)\)\]\)/)
assert.match(pocketRouteShellSource, /pullDistanceRef\.current < 30/)
assert.match(pocketRouteShellSource, /nextDistance >= 30.*runRefresh/)
assert.doesNotMatch(pocketRouteShellSource, /ArrowPath|ArrowDown|Pull to refresh/)
assert.match(pocketRouteShellSource, /pb-\[calc\(7\.5rem\+var\(--pocket-safe-bottom\)\)\]/)
assert.match(pocketRouteShellSource, /top: headerHeight > 0 \? headerHeight : 'var\(--pocket-safe-top\)'/)
assert.match(pocketRouteShellSource, /paddingTop: 16/)
assert.match(pocketRouteShellSource, /--pocket-safe-bottom/)
...
const pocketSessionSplashComponentSource = await readFile(new URL('../src/pocket/components/PocketSessionSplash.tsx', import.meta.url), 'utf8')
const sharedSlideActionSource = await readFile(new URL('../src/components/SlideAction.tsx', import.meta.url), 'utf8')
...
assert.match(pocketBankWithdrawControllerSource, /const prepareApproval = useCallback/)
...
assert.match(pocketHomeControlsSource, /idle: 'Confirm move'/)
assert.doesNotMatch(pocketHomeControlsSource, /bridgeStatus === 'confirming' \|\| bridgeStatus === 'quoting'/)
...
assert.match(pocketSlideActionSource, /<button/)
assert.match(pocketSlideActionSource, /const awaitingApproval = status === 'idle' && optimisticPending/)
assert.match(pocketSlideActionSource, /\? 'Awaiting approval'/)
assert.match(pocketSlideActionSource, /setOptimisticPending\(true\)/)
assert.match(pocketSlideActionSource, /POCKET_PAYMENT_APPROVAL_CANCELLED_EVENT/)
assert.match(pocketSlideActionSource, /setOptimisticPending\(false\)/)
assert.ok(pocketSlideActionSource.indexOf('preparePocketPaymentApproval()') < pocketSlideActionSource.indexOf('requestPocketPaymentApproval()'))
assert.match(pocketSessionSplashComponentSource, /SplashScreen\.hide\(\)/)
assert.match(pocketSessionSplashComponentSource, /nativeRuntime \|\| state !== 'entering'/)
assert.doesNotMatch(pocketSlideActionSource, /Sent - confirming|Confirming send/)
assert.doesNotMatch(pocketSlideActionSource, /Processing\.\.\./)
assert.match(sharedSlideActionSource, /Clock3/)
assert.match(pocketSlideActionSource, /Loader2/)
assert.match(pocketSlideActionSource, /ArrowLeftRight, Banknote, Check, CheckCircle2, Loader2, Send, Wallet/)
assert.match(pocketSlideActionSource, /const ActionIcon = \/swap\|move\|bridge\//)
assert.match(pocketSlideActionSource, /visualStatus === 'successful'[\s\S]*<CheckCircle2/)
assert.match(pocketSlideActionSource, /<ActionIcon/)
assert.doesNotMatch(pocketSlideActionSource, /\bLock\b|lucide-react/)
assert.match(sharedSlideActionSource, /navigator\.vibrate\(8\)/)
assert.doesNotMatch(pocketBridgeControllerSource, /setStatus\('successful'\)/)
assert.match(pocketBridgeControllerSource, /Bridge submitted\. Follow its progress in Activity\./)
assert.match(pocketBridgeControllerSource, /savePocketBridgeTransfer/)
assert.match(pocketBankWithdrawControllerSource, /attempt < maxAttempts/)
assert.match(pocketBankWithdrawControllerSource, /attempt <= 5 \? 2_000 : 5_000/)
assert.match(pocketBankWithdrawControllerSource, /saveActivePocketBankPayout\(prepared\.intentId\)/)
assert.match(pocketBankWithdrawControllerSource, /clearActivePocketBankPayout\(intentId\)/)
assert.match(pocketBankWithdrawControllerSource, /next\.nextAction === 'ensure_liquidity' \|\| next\.nextAction === 'wait_bridge' \|\| next\.nextAction === 'authorize_transfer'/)
assert.doesNotMatch(pocketBankWithdrawControllerSource, /recoverPocketBankWithdrawals/)
assert.match(pocketBankWithdrawControllerSource, /const next = await readPocketBankWithdrawStatus/)
assert.doesNotMatch(pocketBankWithdrawControllerSource, /payoutExpired|payoutDeadline|BANK_PAYOUT_AUTHORIZATION_SAFETY_MS/)
assert.match(pocketBankWithdrawControllerSource, /The payout quote expired before any money was sent\. Your details are still here; tap Confirm again to refresh it\./)
assert.match(pocketBankWithdrawControllerSource, /const session = await getEvmSession\(selectedWallet\.address\)[\s\S]*approvedSession\.current = \{ walletAddress: selectedWallet\.address, session \}/)
assert.match(pocketBankWithdrawControllerSource, /approvedSession\.current\?\.walletAddress === selectedWallet\.address/)
assert.doesNotMatch(pocketBankWithdrawControllerSource, /Payment stopped\. Start a new payment/)
assert.match(pocketBankWithdrawControllerSource, /if \(!recoveredTxHash\)[\s\S]*setError\(''\)[\s\S]*clearStoredOperation\(\)/)
assert.match(pocketBankWithdrawControllerSource, /readActivePocketBankPayoutTransfer\(intentId\)/)
assert.match(pocketBankWithdrawControllerSource, /saveActivePocketBankPayout\(prepared\.intentId, transfer\.txHash\)/)
assert.match(pocketBankWithdrawControllerSource, /registerPocketBankWithdrawTransfer/)
assert.match(pocketBankWithdrawControllerSource, /confirm: false/)
assert.match(pocketBankWithdrawControllerSource, /onAccepted: identifiers =>/)
assert.match(pocketBankWithdrawControllerSource, /saveActivePocketBankPayoutAcceptance\(prepared\.intentId, identifiers\)/)
assert.match(pocketBankWithdrawControllerSource, /reconcileCircleEvmEmailWithdraw/)
assert.doesNotMatch(pocketBankWithdrawControllerSource, /Circle accepted the payout, but no transaction hash was returned/)
assert.match(pocketBankWithdrawControllerSource, /setStatus\('routing'\)/)
assert.match(pocketBankWithdrawControllerSource, /continueAfterRouting/)
assert.match(pocketBankWithdrawControllerSource, /BANK_PAYOUT_OPERATION_KEY/)
assert.match(pocketBankWithdrawControllerSource, /window\.crypto\.subtle\.digest\('SHA-256'/)
assert.match(pocketBankWithdrawControllerSource, /setStatus\('sent'\)/)
assert.match(pocketBankWithdrawControllerSource, /payoutHandoffSucceeded\(next\)/)
assert.match(pocketBankWithdrawControllerSource, /setStatus\('pending'\)/)
assert.match(pocketBankWithdrawControllerSource, /function submittedPayoutStatus/)
assert.match(pocketBankWithdrawControllerSource, /validUntil <= Date\.now\(\)/)
assert.match(pocketBankWithdrawControllerSource, /if \(next\.state === 'expired'\)[\s\S]*setStatus\('pending'\)/)
assert.match(pocketBankWithdrawControllerSource, /if \(next\.state === 'sent'\)[\s\S]*setResult\(next\)[\s\S]*setStatus\('sent'\)/)
assert.doesNotMatch(pocketBankWithdrawControllerSource, /setAmountState\(''\)|setMemoState\(''\)/)
assert.match(pocketBankWithdrawControllerSource, /if \(reconciliation\) void pollUntilSettled/)
assert.match(pocketBankWithdrawControllerSource, /let transactionSubmitted = Boolean\(readActivePocketBankPayoutTransfer/)
assert.match(pocketBankWithdrawControllerSource, /if \(statusRef\.current !== 'idle'\) return/)
assert.match(pocketBankWithdrawControllerSource, /if \(reconciling\.current \|\| polling\.current\) return/)
assert.match(pocketBankWithdrawControllerSource, /if \(!next \|\| activeIntentId\.current !== intentId \|\| cancelled\.current\) return/)
assert.match(pocketBankWithdrawControllerSource, /if \(next\.state === 'sent'\)[\s\S]*activeIntentId\.current = ''[\s\S]*setStatus\('sent'\)/)
assert.doesNotMatch(pocketBankWithdrawControllerSource, /cancelled\.current = false\s+setStatus\('processing'\)/)
assert.match(pocketBankWithdrawControllerSource, /expectedIntentId && activeIntentId\.current !== expectedIntentId/)
assert.match(pocketBankWithdrawControllerSource, /const submittedRoute = \/submitted and is being reconciled\|USDC move submitted/)
assert.match(pocketBankWithdrawControllerSource, /const review = submittedRoute \|\| Boolean\(result\?\.txHash/)
assert.match(circlePocketAppBootSource, /Promise\.allSettled\(\[/)
assert.match(circlePocketAppBootSource, /!readActivePocketBankPayoutTransfer\(intentId\) && !readActivePocketBankPayoutAcceptance\(intentId\)/)
assert.doesNotMatch(circlePocketAppBootSource, /payout\.state === 'sent' \|\| payout\.state === 'refunded'/)
assert.match(circlePocketAppBootSource, /prefetchPocketWalletSnapshot\(\{ email, getAccessToken \}\)/)
assert.match(circlePocketAppBootSource, /restorePocketWalletSession\(email\)/)
assert.match(circlePocketAppBootSource, /Connect your Circle wallet once on this phone/)
assert.doesNotMatch(circlePocketAppBootSource, /unlockPocketBaseWallet\(\{/)
assert.doesNotMatch(circlePocketAppBootSource, /readPocketPaymentSecurity\(getAccessToken\)/)
assert.match(circlePocketAppBootSource, /PocketPaymentSecurityGate/)
assert.doesNotMatch(circlePocketAppBootSource, /enablePocketQuickApproval\(email/)
...
assert.doesNotMatch(pocketWithdrawalControllerSource, /void getEvmSession\(network, wallet\.address\)/)
assert.doesNotMatch(pocketWithdrawalControllerSource, /void getSolanaSession\(wallet\.address\)/)
assert.doesNotMatch(pocketBankWithdrawControllerSource, /const session = await getEvmSession\(wallet\.address\)/)
...
assert.match(pocketHomePageSource, /key: 'arc', label: 'Arc'/)
assert.doesNotMatch(pocketHomePageSource, /label: 'Arc'.*soon: true/)
assert.match(pocketHomePageSource, /POCKET_ROUTES.swap/)
assert.match(pocketHomePageSource, /Request/)
assert.doesNotMatch(pocketHomePageSource, /onRefresh=|refreshing=/)
assert.match(pocketHomePageSource, /<PocketRouteShell[\s\S]*active="home"/)
assert.doesNotMatch(pocketHomePageSource, /PocketX402|App Pay|Smart Wallet|CreateLink|signCircleSolanaTransaction|preparePocketSolanaTransfer|submitPocketSolanaTransfer|executePocketEvmTransfer|linkPocketWallet/i)
...
assert.match(pocketHomeOverviewSource, /key: 'arc', label: 'Arc'/)
assert.doesNotMatch(pocketHomeOverviewSource, /key: 'arc'[^\n]*comingSoon: true/)
...
const pocketEmailLoginSource = await readFile(new URL('../src/pocket/components/PocketEmailLogin.tsx', import.meta.url), 'utf8')
const officialPrivyMark = await readFile(new URL('../public/privy-mark-logo.png', import.meta.url))
...
assert.match(pocketLandingPageSource, /Checking your Pocket/)
assert.match(pocketLandingPageSource, /authenticated, profile\.busy, profile\.loadError, profile\.loaded, profile\.profile, splashState/)
assert.doesNotMatch(pocketLandingPageSource, /if \(authenticated\) \{[\s\S]{0,260}<CPurseIcon/)
assert.doesNotMatch(pocketLandingPageSource, /Open Pocket|enterAfterLogin/)
assert.doesNotMatch(pocketLandingPageSource, /profile\.save\(|What is your first name\?|And your last name\?|firstName|lastName/)
assert.match(pocketLandingPageSource, /<PocketEmailLogin \/>/)
assert.match(pocketLandingPageSource, /<PocketAuthBrand markRef={heroLogoRef} \/>/)
assert.match(pocketLandingPageSource, /usePocketLightSurface\(\)/)
assert.doesNotMatch(pocketLandingPageSource, /h-\[164px\]|sm:h-\[190px\]/)
assert.doesNotMatch(pocketLandingPageSource, /flex flex-1 flex-col items-center justify-center py-7/)
assert.match(pocketLandingPageSource, /<section className=\{`mt-8 space-y-2\.5/)
assert.doesNotMatch(pocketLandingPageSource, /PrivyConnectButton|pocket-landing-sign-in/)
assert.doesNotMatch(pocketLandingPageSource, /PocketThemeToggle|dark:/)
assert.match(pocketEmailLoginSource, /useLoginWithEmail\(\)/)
assert.match(pocketEmailLoginSource, /sendCode\(\{ email: normalizedEmail \}\)/)
assert.match(pocketEmailLoginSource, /loginWithCode\(\{ code \}\)/)
assert.match(pocketEmailLoginSource, /autoComplete="one-time-code"/)
assert.match(pocketEmailLoginSource, /\{productName\} will never ask you to share this code\./)
assert.doesNotMatch(pocketEmailLoginSource, /<PocketAuthBrand/)
assert.match(pocketEmailLoginSource, /grid grid-cols-6 gap-2/)
assert.match(pocketEmailLoginSource, /text-2xl font-black/)
assert.match(pocketEmailLoginSource, /bg-white text-gray-950/)
assert.match(pocketEmailLoginSource, /border-blue-600[^']{0,80}ring-4 ring-blue-500\/15/)
assert.match(pocketEmailLoginSource, /Secured by Privy/)
assert.match(pocketEmailLoginSource, /privy-mark-logo\.png/)
assert.equal(createHash('sha256').update(officialPrivyMark).digest('hex'), '2b09ddddff05b2d86f5d1adb44630eaa1c0f8b530edb9a657509ac88145ea547')
assert.doesNotMatch(pocketEmailLoginSource, /CPurseIcon|Pocket by Hash PayLink|privy-email-logo/)
assert.match(pocketEmailLoginSource, /<span>Secured by<\/span>[\s\S]{0,180}privy-mark-logo\.png[\s\S]{0,120}<span>Privy<\/span>/)
assert.doesNotMatch(pocketEmailLoginSource, /to="\/docs\/(?:terms|privacy)"/)
assert.doesNotMatch(pocketEmailLoginSource, /aria-label="Pocket"/)
assert.doesNotMatch(pocketLandingPageSource, /Your money, ready to move/)
assert.match(circlePocketAppBootSource, /Sign in to your Circle Pocket wallet/)
assert.match(circlePocketAppBootSource, /Retry Circle sign in/)
assert.match(circlePocketAppBootSource, /<PocketProgressDots label='Opening Circle wallet sign in' \/>/)
assert.match(circlePocketAppBootSource, /usePocketLightSurface\(\)/)
assert.match(pocketAuthBrandSource, /Pocket by Hash PayLink/)
assert.match(pocketAuthBrandSource, /<CPurseIcon/)
assert.match(pocketAuthBrandSource, /markRef/)
assert.match(pocketAuthBrandSource, />By Hash PayLink</)
assert.match(pocketProgressDotsSource, /\[0, 160, 320\]/)
assert.match(pocketProgressDotsSource, /motion-reduce:animate-none/)
assert.match(pocketLightSurfaceSource, /lightSurfaceCount/)
assert.match(pocketLightSurfaceSource, /root\.classList\.remove\('dark'\)/)
assert.match(pocketLightSurfaceSource, /if \(restoreDarkTheme\) root\.classList\.add\('dark'\)/)
assert.match(themeContextSource, /dataset\.pocketLightSurface === 'true'/)
assert.match(themeContextSource, /dark && !pocketLightSurface/)
assert.match(pocketPaymentSecurityGateSource, /<PocketSecuritySurface(?: scroll)?>/)
assert.match(pocketEmailLoginSource, /createPortal\(/)
assert.match(pocketEmailLoginSource, /document\.body/)
assert.match(pocketLandingPageSource, /to="\/docs\/terms"/)
assert.match(pocketLandingPageSource, /to="\/docs\/privacy"/)
assert.doesNotMatch(pocketEmailLoginSource, /dark:/)
assert.doesNotMatch(pocketEmailLoginSource, /console\.|localStorage|sessionStorage/)
assert.match(termsDocsSource, /Last updated August 19, 2026/)
assert.match(termsDocsSource, /Bank payouts and local-currency services/)
assert.match(termsDocsSource, /Bills and digital services/)
assert.match(termsDocsSource, /Agent Hash and automated assistance/)
assert.match(privacyDocsSource, /Last updated August 19, 2026/)
assert.match(privacyDocsSource, /Google Firebase/)
assert.match(privacyDocsSource, /raw PIN/)
assert.match(privacyDocsSource, /Nigeria Data Protection Act 2023/)
const pocketAccountMenuSource = await readFile(new URL('../src/pocket/components/PocketAccountMenu.tsx', import.meta.url), 'utf8')
const pocketIconsSource = await readFile(new URL('../src/pocket/components/PocketIcons.ts', import.meta.url), 'utf8')
assert.match(pocketIconsSource, /border-t-transparent/)
assert.match(pocketIconsSource, /animate-spin/)
assert.doesNotMatch(pocketIconsSource, /ArrowPathIcon as Loader2/)
assert.match(pocketAccountMenuSource, /<PocketAvatar avatarId=/)
assert.match(pocketAccountMenuSource, /navigator\.clipboard\.writeText\(current\.pocketId\)/)
assert.match(pocketAccountMenuSource, /View profile/)
assert.match(pocketAccountMenuSource, /Edit Pocket ID/)
assert.match(pocketAccountMenuSource, /PocketThemeToggle/)
assert.doesNotMatch(pocketAccountMenuSource, /Sign out/)
const pocketProfilePageSource = await readFile(new URL('../src/pocket/pages/PocketProfilePage.tsx', import.meta.url), 'utf8')
assert.match(pocketProfilePageSource, /navigator\.clipboard\.writeText\(current\.pocketId\)/)
...
assert.match(pocketMoveUsdcPageSource, /<PocketRouteShell active="home"/)
assert.match(pocketMoveUsdcPageSource, /<PocketFlowHeader title="Request"/)
assert.doesNotMatch(pocketMoveUsdcPageSource, /<PocketFlowHeader title="Receive"/)
assert.match(pocketMoveUsdcPageSource, /<PocketPayLinkReadyPanel/)
assert.match(pocketMoveUsdcPageSource, /<PayLinkShareSheet/)
assert.match(pocketMoveUsdcPageSource, /Sign in to keep requests, collections, payments, and receipts together\./)
assert.match(pocketMoveUsdcPageSource, /Request USDC from one Pocket user\. Payment stays inside Pocket\./)
assert.match(pocketMoveUsdcPageSource, /No public link or QR code is created\./)
...
const pocketVerifiedBankFieldsSource = await readFile(new URL('../src/pocket/features/move/PocketVerifiedBankFields.tsx', import.meta.url), 'utf8')
assert.match(pocketVerifiedBankFieldsSource, /<span className="absolute right-3 top-1\/2[^"]*-translate-y-1\/2[^"]*"><Loader2 className="h-4 w-4"/)
assert.doesNotMatch(pocketVerifiedBankFieldsSource, /<Loader2 className="[^"]*-translate-y-1\/2/)
...
assert.match(pocketMoveBankPageSource, /<PocketRouteShell active="home"/)
assert.match(pocketMoveBankPageSource, /routeMode === 'request' \? 'Request' : 'Bank payout'/)
assert.doesNotMatch(pocketMoveBankPageSource, /routeMode === 'request' \? 'Receive'/)
assert.match(pocketMoveBankPageSource, /Local-currency collection/)
assert.match(pocketMoveBankPageSource, /Collection country/)
assert.match(pocketMoveBankPageSource, /Ghana/)
assert.match(pocketMoveBankPageSource, /Kenya/)
assert.match(pocketMoveBankPageSource, /Direct Bank Payout/)
assert.match(pocketMoveBankPageSource, /Withdrawal network/)
assert.match(pocketMoveBankPageSource, /Confirm payout/)
assert.match(pocketMoveBankPageSource, /usePocketBankWithdrawController\(\{/)
assert.match(pocketMoveBankPageSource, /usePocketPaymentLiquidityController\(\{/)
assert.match(pocketMoveBankPageSource, /const getLiquidityEvmSession = useCallback/)
assert.match(pocketMoveBankPageSource, /const getLiquiditySolanaSession = useCallback/)
assert.match(pocketMoveBankPageSource, /direct\.failRouting\(reason, intentId\)/)
assert.match(pocketMoveBankPageSource, /onPrepare=\{direct\.prepareApproval\}/)
assert.match(pocketMoveBankPageSource, /readPocketBankWithdrawRoute/)
assert.match(pocketMoveBankPageSource, /USDC moving to Base/)
assert.match(pocketMoveBankPageSource, /direct\.status === 'sent' \|\| direct\.status === 'pending'/)
assert.match(pocketMoveBankPageSource, /outcome=\{direct\.status === 'sent' \? 'handed-off' : 'pending'\}/)
assert.match(pocketMoveBankPageSource, /\{ allowPending: true \}/)
assert.match(pocketMoveBankPageSource, /\{authenticated && bank\.profileVerified && <fieldset disabled=\{mode === 'withdraw' && directLocked\}/)
assert.doesNotMatch(pocketMoveBankPageSource, /Fund with Bank|usePocketBankFundController|\/api\/pocket\/bank-fund/)
...
assert.match(pocketMoveBankPageSource, /Nigeria collections currently use Base USDC checkout\./)
assert.match(pocketMoveBankPageSource, /Sign in to keep collections, bank payouts, receipts, and support connected to your Pocket\./)
assert.match(pocketPaymentLiquidityControllerSource, /input\.persistence\?\.read/)
assert.match(pocketPaymentLiquidityControllerSource, /input\.persistence\?\.start/)
assert.match(pocketPaymentLiquidityControllerSource, /input\.persistence\?\.update/)
...
assert.match(pocketBankReceiveControllerSource, /client_origin: pocketRuntimeOrigin\(\)/)
assert.match(pocketBankReceiveControllerSource, /allowThirdPartyAccount && staleOwnerNameRejection && Boolean\(accountName\)/)
assert.match(pocketBankReceiveControllerSource, /allowThirdPartyAccount && staleOwnerNameRejection \? '' : error/)
...
assert.match(pocketMovePosPageSource, /<PocketRouteShell active="home"/)
...
assert.match(circlePocketAppSource, /route\?\.section === 'home'[\s\S]*<PocketHomePage/)
assert.match(circlePocketAppSource, /route\?\.section === 'profile'[\s\S]*<PocketProfilePage/)
assert.doesNotMatch(circlePocketAppSource, /PocketX402Page/)
...
assert.match(pocketX402PageSource, /Verify your App Pay wallet/)
assert.match(pocketX402PageSource, /Verification needed/)
assert.match(pocketX402PageSource, /Confirm this Circle wallet before adding App Pay funds\./)
...
assert.match(pocketX402ControllerSource, /pocketX402ReconnectRequired\(reason\)/)
assert.match(pocketX402ControllerSource, /activationKey\.current = ''/)
assert.match(pocketX402ControllerSource, /setWalletMode\('login'\)/)
assert.match(pocketX402ControllerSource, /connected: false/)
assert.match(pocketX402ControllerSource, /action: 'init'/)
assert.match(pocketX402ControllerSource, /setWalletStep\('otp'\)/)
...
assert.match(pocketMarketplacePanelSource, /to=\{`\$\{POCKET_BASE_PATH\}\/activity`\}/)
...
assert.match(pocketRouteShellSource, /scrollPaddingBottom: 'calc\(7\.5rem \+ var\(--pocket-safe-bottom\)\)'/)
assert.match(pocketRouteShellSource, /data-pocket-scroller/)
assert.match(pocketRouteShellSource, /scrollPaddingTop: 16/)
...
assert.doesNotMatch(circlePocketAppSource, /Restoring your Pocket|animate-spin/)
assert.doesNotMatch(circlePocketAppSource, /authenticated && \(!profile\.loaded/)
assert.doesNotMatch(pocketHomePageSource, /PocketLoadingState/)
assert.doesNotMatch(pocketHomePageSource, /!wallets\.resolved \|\| !activity\.resolved/)
assert.match(pocketHomePageSource, /const balancesVisible = balancesReady/)
assert.match(pocketHomePageSource, /!activityReady \? <PocketRecentActivitySkeleton \/>/)
assert.match(pocketLoadingStateSource, /if \(active === 'activity'\) return <PocketActivityLoadingState \/>/)
assert.match(pocketHomePageSource, /Recent activity/)
const pocketBalancesSource = await readFile(new URL('../api/pocket/balances.ts', import.meta.url), 'utf8')
assert.match(pocketBalancesSource, /DIRECT_BALANCE_TIMEOUT_MS = 2_000/)
assert.match(pocketBalancesSource, /FALLBACK_BALANCE_TIMEOUT_MS = 1_500/)
assert.doesNotMatch(pocketBalancesSource, /BALANCE_TIMEOUT_MS = 10_000/)
...
assert.match(circleSolanaEmailWalletSource, /export async function resumeCircleSolanaEmailWallet/)
assert.match(circleEvmEmailWalletSource, /circleRuntimeUrl\('\/api\/circle-solana-email'\)/)
assert.match(circleSolanaEmailWalletSource, /circleRuntimeUrl\('\/api\/circle-solana-email'\)/)
assert.match(circleEvmEmailWalletSource, /POCKET_NATIVE_ORIGIN = 'https:\/\/pocket\.hashpaylink\.com'/)
...
assert.match(circleEvmEmailWalletSource, /Once Circle has accepted the signed challenge/)
assert.match(circleEvmEmailWalletSource, /pollTransactionHash\(params\.session, transactionId, 5_000\)/)
assert.match(circleEvmEmailWalletSource, /record\.errorReason, record\.errorDetails, record\.errorCode/)
assert.match(circleEvmEmailWalletSource, /if \(state\.includes\('FAILED'\)\) throw new Error\(transactionFailureMessage\(data\.transaction\)\)/)
assert.match(circleSolanaEmailWalletSource, /capturePocketViewport\(\)/)
assert.match(circleSolanaEmailWalletSource, /pollCircleSolanaTransaction\(params\.session\.userToken, transactionId, 5_000\)/)
assert.match(circleSolanaEmailWalletSource, /correlationIds/)
assert.match(circleSolanaEmailWalletSource, /A status lookup outage/)
assert.match(circleSolanaEmailWalletSource, /export async function reconcileCircleSolanaTransfer/)
assert.doesNotMatch(circleSolanaEmailWalletSource, /Circle accepted the Solana transfer and is reconciling it\. Do not retry yet\./)
assert.doesNotMatch(circleEvmEmailWalletSource, /fonts\.googleapis|fonts\.gstatic/)
assert.doesNotMatch(circleSolanaEmailWalletSource, /fonts\.googleapis|fonts\.gstatic/)
...
assert.match(pocketMoveUsdcSource, /usePocketWallets\(\{ authenticated, email, getAccessToken \}\)/)
...
const pocketSendPageSource = await readFile(new URL('../src/pocket/pages/PocketSendPage.tsx', import.meta.url), 'utf8')
const pocketNotificationsPageSource = await readFile(new URL('../src/pocket/pages/PocketNotificationsPage.tsx', import.meta.url), 'utf8')
assert.match(pocketNotificationsPageSource, /item\.createdAt/)
assert.match(pocketNotificationsPageSource, />Sent \{new Date\(item\.createdAt\)/)
assert.match(pocketSendPageSource, /usePocketPaymentLiquidityController/)
assert.match(pocketSendPageSource, /restoreOperations: Boolean\(requestId\) \|\| mode === 'address'/)
assert.doesNotMatch(pocketSendPageSource, /paymentRequest && requestLiquidity\.busy \?/)
assert.match(pocketSendPageSource, /readPocketRequestRoute/)
assert.match(pocketSendPageSource, /startPocketRequestRoute/)
assert.match(pocketSendPageSource, /updatePocketRequestRoute/)
assert.match(pocketSendPageSource, /requestLiquidity\.ensureLiquidity\(\)/)
assert.match(pocketSendPageSource, /balanceOverride: Number\(paymentRequest\.amount\)/)
assert.match(pocketSendPageSource, /preserveForm: true/)
assert.match(pocketSendPageSource, /const requestPaid = requestConfirmed \|\| requestAccepted/)
assert.match(pocketSendPageSource, /Your payment was sent successfully\. It will appear in Activity shortly\./)
assert.match(pocketSendPageSource, /onConfirm=\{submitPayment\}/)
assert.match(pocketSendPageSource, /paymentRequest\.createdAt/)
assert.match(pocketSendPageSource, /requestLiquidity\.route\?\.kind === 'bridge'/)
assert.match(pocketWithdrawalControllerSource, /if \(!restoreOperations \|\| network !== 'solana'/)
assert.doesNotMatch(pocketWithdrawalControllerSource, /void getSolanaSession\(wallet\.address\)/)
assert.doesNotMatch(pocketBillsPanelSource, /bills\.status === 'confirming' \|\| paymentRouting\?\.status === 'checking'/)
const pocketSwapPageSource = await readFile(new URL('../src/pocket/pages/PocketSwapPage.tsx', import.meta.url), 'utf8')
assert.doesNotMatch(pocketSwapPageSource, /swap\.status === 'confirming' \|\| swap\.status === 'quoting'/)
assert.match(pocketSwapPageSource, /disabled: swap\.status === 'quoting' \? 'Getting live quote'/)
...
assert.match(pocketMoveUsdcSource, /Group collection/)
assert.match(pocketMoveUsdcSource, /flow === 'request' \? <>/)
assert.match(pocketMoveUsdcSource, /<PocketPayerNetworkPanel/)
assert.match(pocketReceiveMethodSource, /Receive with address/)
assert.match(pocketReceiveMethodSource, /Pocket wallet/)
...
assert.doesNotMatch(pocketTopSwitchSource, /Smart Wallet|App Pay|x402/)
assert.match(pocketTopSwitchSource, /from '.\/PocketIcons'/)
assert.match(pocketTopSwitchSource, /bg-white\/95/)
...
assert.match(ngPosSource, /'deposited', 'pending', 'fulfilling', 'fulfilled', 'validated'/)
...
const developerPortalSource = await readFile(new URL('../src/pages/DeveloperPortalPage.tsx', import.meta.url), 'utf8')
const agentCheckoutPageSource = await readFile(new URL('../src/pages/AgentCheckoutPage.tsx', import.meta.url), 'utf8')
const unifiedReceiptSource = await readFile(new URL('../src/components/UnifiedReceipt.tsx', import.meta.url), 'utf8')
const paymentReceiptPdfSource = await readFile(new URL('../src/lib/paymentReceiptPdf.ts', import.meta.url), 'utf8')
const paymentReceiptSuccessSource = await readFile(new URL('../src/pocket/components/PocketPaymentSuccess.tsx', import.meta.url), 'utf8')
const receiptEventRegistrySource = await readFile(new URL('../api/event-registry.ts', import.meta.url), 'utf8')
const receiptApiSource = await readFile(new URL('../api/receipt.ts', import.meta.url), 'utf8')
...
assert.match(paymentPageSource, /<SlideAction/)
assert.match(paymentPageSource, /pocketMovePayRetryBlocked/)
assert.match(paymentPageSource, /Check payment activity/)
assert.match(paymentPageSource, /submitted and is being reconciled\|still moving\|destination balance is still refreshing/)
assert.match(paymentPageSource, /resolveHostedCheckoutKind\(searchParams\)/)
assert.match(paymentPageSource, /const hostedEvmRecipients:/)
assert.match(paymentPageSource, /const HOSTED_CHECKOUT_CHAINS: ChainKey\[\] = \['base', 'arbitrum', 'arc'\]/)
assert.match(paymentPageSource, /hasHostedCheckoutParam \? HOSTED_CHECKOUT_CHAINS : CHAINS/)
assert.match(paymentPageSource, /const humanCheckoutNetworkOptions = \[/)
assert.match(paymentPageSource, /label: value === 'arc' \? 'Arc' : CHAIN_META\[value\]\.label/)
assert.match(paymentPageSource, /\.\.\.\(hasHostedCheckoutParam && !availableChains\.includes\('solana'\)/)
assert.ok(
  paymentPageSource.indexOf('const hasHostedCheckoutParam') < paymentPageSource.indexOf('const humanCheckoutNetworkOptions'),
...
assert.doesNotMatch(paymentPageSource, /<img src="\/hash-logo-transparent\.png"[^>]*>\s*<span>Prepare naira payout<\/span>/)
assert.match(paymentPageSource, /<span>Continue to pay<\/span>/)
assert.match(paymentPageSource, /const hasScopedEvmRecipients = Object\.values\(hostedEvmRecipients\)\.some\(Boolean\)/)
assert.match(paymentPageSource, /hasScopedEvmRecipients \? hostedEvmRecipients\[candidate\] \|\| '' : evmParam/)
assert.match(paymentPageSource, /const resolvedEvm = evmRecipientForChain\(chain\)/)
assert.match(paymentPageSource, /isHostedCheckout && hostedIntentStatus !== 'verified'/)
assert.match(paymentPageSource, /expected\.pathname !== current\.pathname \|\| expected\.search !== current\.search/)
assert.match(paymentPageSource, /window\.location\.replace\(`\$\{expected\.pathname\}\$\{expected\.search\}`\)/)
assert.match(paymentPageSource, /Checkout verification failed/)
assert.match(paymentPageSource, /setPaymentAttemptStarted\(true\)/)
assert.match(paymentPageSource, /const \[payMode\] = useState<'wallet' \| 'direct'>\('wallet'\)/)
assert.doesNotMatch(paymentPageSource, /Pay another way|canDirectSend|setPayMode|modeParam/)
assert.match(paymentPageSource, /args:\s*\{ from: expectedPayer, to: watchTarget \}/)
assert.match(paymentPageSource, /args:\s*\{ from: expectedPayer, to: target \}/)
assert.match(paymentPageSource, /payer: expectedPayer/)
assert.match(paymentPageSource, /Continue to \{hostedMerchantName \|\| 'platform'\}/)
assert.doesNotMatch(paymentPageSource, /Returning to .* in 6 seconds|Return to service now/)
assert.match(paymentPageSource, /receiptReady && !isPolymarketBridge && !polymarketReturnToAgentHash/)
assert.match(paymentPageSource, /<UnifiedReceipt receipt=\{paymentReceipt \?\? undefined\} receiptId=\{paymentReceiptId\} \/>/)
assert.doesNotMatch(paymentPageSource, /Share receipt|View receipt|href=\{`\/receipt\//)
assert.match(paymentPageSource, /hostedMerchantLogo.*merchantLogo/)
assert.match(developerPortalSource, /Checkout brand mark/)
assert.match(developerPortalSource, /checkout and receipts/)
assert.match(developerPortalSource, /environmentSection\('test', 'Test keys', `Sandbox keys do not authorize mainnet\./)
assert.match(developerPortalSource, /environmentSection\('live', 'Live keys', `Base, Arbitrum, and configured Arc mainnet routes\./)
assert.match(developerPortalSource, /function CheckoutModePicker/)
assert.match(developerPortalSource, /Human and agentic checkouts use separate projects, policies, and API keys\./)
assert.match(developerPortalSource, /Back to projects/)
assert.match(developerPortalSource, /No projects are linked to this sign-in\./)
assert.match(developerPortalSource, /Arc Agreements · Private pilot/)
assert.doesNotMatch(developerPortalSource, /Funding stays unavailable until the contract layer is reviewed\./)
...
assert.match(agentCheckoutPageSource, /fetch\(`\/api\/receipt\?id=/)
assert.match(unifiedReceiptSource, /ReceiptSurface = 'details' \| 'receipt' \| null/)
assert.match(unifiedReceiptSource, /createPaymentReceiptPdf\(receipt\)/)
assert.match(unifiedReceiptSource, /createPaymentReceiptImage\(receipt\)/)
assert.match(unifiedReceiptSource, /View details/)
assert.match(unifiedReceiptSource, /Share receipt/)
assert.match(unifiedReceiptSource, /Share as image/)
assert.match(unifiedReceiptSource, /Share as PDF/)
assert.match(unifiedReceiptSource, /function receiptDataUrlBlob/)
assert.doesNotMatch(unifiedReceiptSource, /fetch\(dataUrl\)/)
assert.match(unifiedReceiptSource, /style=\{\{ top: 'var\(--pocket-safe-top\)' \}\}/)
assert.match(unifiedReceiptSource, /Payment pending/)
assert.doesNotMatch(unifiedReceiptSource, /fixed inset-0 z-\[140\]/)
assert.match(unifiedReceiptSource, />Done<\/button>/)
assert.doesNotMatch(unifiedReceiptSource, /<ArrowLeft/)
assert.match(unifiedReceiptSource, /navigator\.share/)
assert.match(unifiedReceiptSource, /paymentReceiptFileName\(receipt\)/)
assert.match(unifiedReceiptSource, /paymentReceiptImageFileName\(receipt\)/)
assert.doesNotMatch(unifiedReceiptSource, /window\.open/)
assert.match(unifiedReceiptSource, /\/api\/x402\/receipt\?id=/)
assert.match(unifiedReceiptSource, /\/api\/receipt\?id=/)
assert.match(unifiedReceiptSource, /paymentReceiptView/)
assert.match(unifiedReceiptSource, /Powered by Hash PayLink/)
assert.match(unifiedReceiptSource, /<dl/)
assert.match(paymentReceiptSuccessSource, /Payment successful/)
assert.match(paymentReceiptSuccessSource, /Payment pending/)
assert.match(paymentReceiptSuccessSource, /USDC reached the payout recipient/)
assert.match(paymentReceiptSuccessSource, /payment window elapsed before the recipient detected your USDC/)
assert.doesNotMatch(paymentReceiptSuccessSource, /Paycrest/i)
assert.match(paymentReceiptSuccessSource, /<UnifiedReceipt receipt=\{receipt\}/)
assert.match(paymentReceiptSuccessSource, /title = 'Bank payout'/)
assert.match(paymentReceiptSuccessSource, /overflow-hidden/)
assert.match(paymentReceiptSuccessSource, /style=\{\{ top: 'var\(--pocket-safe-top\)' \}\}/)
assert.doesNotMatch(paymentReceiptSuccessSource, /fixed inset-0/)
assert.match(paymentReceiptPdfSource, /const footer = 'Powered by Hash PayLink'/)
assert.match(paymentReceiptPdfSource, /const brand = paymentReceiptBrand\(receipt\)/)
const pocketMarkSource = paymentReceiptPdfSource.slice(paymentReceiptPdfSource.indexOf('function drawPocketMark'), paymentReceiptPdfSource.indexOf('function shortPdfValue'))
assert.match(pocketMarkSource, /ctx\.arc\(/)
assert.doesNotMatch(pocketMarkSource, /roundRect\(/)
assert.match(receiptEventRegistrySource, /brandName = checkout\.merchantName/)
assert.match(receiptEventRegistrySource, /brandImageUrl = checkout\.brandImageUrl \|\| ''/)
assert.match(receiptApiSource, /brandKind: receipt\.brandName \? 'partner' : 'hashpaylink'/)
const archiveReceiptSource = receiptEventRegistrySource.slice(receiptEventRegistrySource.indexOf('function archiveRecordFor'), receiptEventRegistrySource.indexOf('function scheduleArchivePayment'))
assert.doesNotMatch(archiveReceiptSource, /brandName|brandImageUrl/)
assert.match(streamPayAppSource, /path="receipt\/:activityId" element=\{<X402Receipt \/>\}/)
assert.match(createStreamFormSource, /<UnifiedReceipt receipt=\{streamReceipt\}/)
assert.doesNotMatch(createStreamFormSource, /Share receipt|handleShareReceipt|handleOpenReceipt/)
assert.match(pocketActivityPanelSource, /pocketActivityReceipt\(row\)/)
assert.match(pocketActivityPanelSource, /<UnifiedReceipt receipt=\{receipt\}/)
assert.match(pocketActivityPanelSource, /pocketActivityStatus\(row\)/)
assert.match(pocketActivityPanelSource, /expandedActivityId/)
assert.match(pocketActivityPanelSource, /activityKind\(row\) !== 'bank'[\s\S]*?pocketActivityStatus\(row\) !== 'status unavailable'/)
assert.match(pocketActivityPanelSource, /const collapsible = Boolean\(receipt \|\| row\.bridge\)/)
assert.match(pocketActivityPanelSource, /aria-expanded=/)
assert.doesNotMatch(pocketActivityPanelSource, /expandedReceipt|Receipt details/)
assert.match(dashboardSource, /paymentReceiptView\(/)
assert.match(dashboardSource, /@page \{ size: 80mm auto;/)
assert.match(dashboardSource, /<UnifiedReceipt receiptId=\{rowReceiptId\(row\)\} compact \/>/)
assert.match(agentCheckoutPageSource, /Continue to \{checkout\.merchantName\}/)
assert.match(paymentPageSource, /hostedConfirmationStatus !== 'verified'/)
assert.match(paymentPageSource, /source: hosted \? 'hosted-checkout' : 'paylink'/)
assert.match(paymentPageSource, /Payment is on-chain; checkout verification is pending\. Do not pay again\./)
assert.match(paymentPageSource, /Retry verification/)
assert.doesNotMatch(paymentPageSource, /sendCircleEvmEmailWithdraw|\(\['fund', 'withdraw'\] as const\)/)
assert.match(hostedCheckoutSource, /'usdc_request' \| 'pos' \| 'bank_request' \| 'wallet_funding' \| 'service'/)
assert.match(hostedCheckoutSource, /action: 'Slide to pay'/)
assert.match(hostedCheckoutSource, /steps: \['Check service', 'Slide to pay', 'Return to service'\]/)
assert.doesNotMatch(hostedCheckoutSource, /Access granted/)
assert.match(slideActionSource, /'successful' \| 'error'/)
assert.match(slideActionSource, /labels\?\.error \?\? 'Payment failed'/)
assert.match(slideActionSource, /const progressRef = useRef\(0\)/)
assert.match(slideActionSource, /const activationLocked = useRef\(false\)/)
assert.match(slideActionSource, /if \(activationLocked\.current\) return/)
assert.match(slideActionSource, /if \(progressRef\.current >= 0\.82\)/)
assert.match(pocketSlideActionSource, /idle: 'Confirm withdrawal'/)
assert.match(pocketSlideActionSource, /<button/)
assert.doesNotMatch(pocketSlideActionSource, /onPointerMove|setPointerCapture/)
assert.match(pocketSlideActionSource, /disabled: 'Enter withdrawal details'/)
assert.match(pocketSlideActionSource, /const mergedLabels = \{ \.\.\.POCKET_DEFAULT_LABELS, \.\.\.labels \}/)
...
assert.match(paymentPageSource, /pocketMovePayRetryBlocked \|\| isConfirming/)
assert.match(paymentPageSource, /pocketMovePayBusy \|\| isWalletPending/)
assert.doesNotMatch(paymentPageSource, /walletFundingConfirming[\s\S]{0,300}circleEvmPaymentProcessing/)
assert.match(agentWalletSource, /CIRCLE_GATEWAY_BALANCE_API_MAINNET/)
assert.match(agentWalletSource, /fetch\(`\$\{apiBase\}\/v1\/balances`/)
assert.match(agentWalletSource, /readAgentGatewayBalance\(record\.walletAddress, params\.network, serviceKey\)/)
assert.match(agentWalletSource, /const method = 'direct'/)
assert.doesNotMatch(agentWalletSource, /const method = params\.network === 'arc' \? 'direct' : 'eco'/)
assert.match(agentWalletSource, /sources: \[\{ depositor: address, domain \}\]/)
assert.match(agentWalletSource, /readGatewayBalance\(gatewayDepositor, params\.network\)/)
const paymentTxLookupSource = await readFile(new URL('../api/payment-tx-lookup.ts', import.meta.url), 'utf8')
const eventRegistrySource = await readFile(new URL('../api/event-registry.ts', import.meta.url), 'utf8')
assert.match(eventRegistrySource, /source === 'hosted-checkout'/)
assert.match(eventRegistrySource, /const paymentOption = evmChain \? hostedCheckoutPaymentOption\(checkout, evmChain\) : null/)
assert.match(eventRegistrySource, /payer,\s*recipient: paymentOption\.recipient,\s*minAmount: minimumAmount,\s*notBefore: checkout\.createdAt,\s*notAfter: checkout\.expiresAt/)
assert.match(eventRegistrySource, /amount = verifiedTransfer\.amount/)
assert.match(eventRegistrySource, /if \(!verifiedTransfer\.confirmedAt\) throw new Error/)
assert.match(eventRegistrySource, /markHostedCheckoutPaid\(\{[\s\S]*?confirmedAt: verifiedTransfer\.confirmedAt/)
assert.match(eventRegistrySource, /network: evmChain/)
assert.match(paymentTxLookupSource, /const requestedFromBlock = readOptionalBlock\(body\.fromBlock\)/)
assert.match(paymentTxLookupSource, /const payerTopic = pad\(payer as Address/)
assert.match(paymentTxLookupSource, /topics: \[TRANSFER_TOPIC, payerTopic, recipientTopic\]/)
...
assert.match(telegramPaymentLinksSource, /import \{ askPocketAgent \} from '\.\.\/pocket\/api\/pocketAgentClient'/)
assert.match(telegramPaymentLinksSource, /if \(helperMode === 'circle-pocket'\) \{[\s\S]*await askPocketAgent\(\{/)
assert.match(telegramPaymentLinksSource, /threadId: activeHelperThreadId/)
assert.match(telegramPaymentLinksSource, /actionLink: \{ label: 'Open Circle Pocket', url: '\/move\/usdc' \}/)
assert.doesNotMatch(telegramPaymentLinksSource, /actionLink: \{ label: 'Open Circle Pocket', url: 'https:\/\/pocket\.hashpaylink\.com\/move\/usdc' \}/)
...
const pocketPushNotificationsSource = await readFile(new URL('../src/pocket/hooks/usePocketPushNotifications.ts', import.meta.url), 'utf8')
assert.match(pocketPushNotificationsSource, /VITE_POCKET_PUSH_ENABLED === 'true'/)
assert.match(pocketPushNotificationsSource, /if \(!POCKET_PUSH_ENABLED \|\| !userEnabled \|\| !Capacitor\.isNativePlatform\(\)/)
assert.match(pocketPushNotificationsSource, /if \(permission\.receive === 'granted'\) await PushNotifications\.register\(\)/)
assert.match(pocketPushNotificationsSource, /fetch\(POCKET_API\.pushDevices/)
assert.match(pocketPushNotificationsSource, /rememberPocketPushToken\(registration\.value\)/)
assert.doesNotMatch(pocketPushNotificationsSource, /if \(registered\) void unregister/)
const pocketPushDevicesSource = await readFile(new URL('../api/pocket/push-devices.ts', import.meta.url), 'utf8')
assert.match(pocketPushDevicesSource, /device\.platform === 'ios'/)
assert.match(pocketPushDevicesSource, /https:\/\/api\.push\.apple\.com/)
assert.match(pocketPushDevicesSource, /https:\/\/api\.sandbox\.push\.apple\.com/)
assert.match(pocketPushDevicesSource, /APPLE_APNS_KEY_ID/)
assert.match(pocketPushDevicesSource, /APPLE_APNS_PRIVATE_KEY/)
assert.match(pocketPushDevicesSource, /dsaEncoding: 'ieee-p1363'/)
assert.match(pocketPushDevicesSource, /if \(!serviceAccount\) return \{ token: device\.token/)
