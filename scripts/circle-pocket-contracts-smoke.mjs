...
// Durable activity behavior is exercised with synthetic source failures, restarts,
// concurrent readers, and real browser account switches in the dedicated suites.
assert.match(pocketActivityHookSource, /refreshPocketActivity/)
assert.doesNotMatch(pocketActivityHookSource, /\/api\/ng-pos|fetch\(|submit|transfer/i)
...
assert.match(pocketWithdrawalControllerSource, /handedOff = confirmed \|\| Boolean\(result\.txHash\) \|\| circleAccepted/)
...
assert.match(pocketWithdrawalControllerSource, /confirm: true/)
...
assert.doesNotMatch(pocketWithdrawalControllerSource, /if \(operation\.state === 'accepted'\) \{\s*clearSolanaOperation\(\)\s*return/)
assert.doesNotMatch(pocketWithdrawalControllerSource, /if \(operation\.state === 'accepted'\) \{\s*clearEvmOperation\(\)\s*return/)
