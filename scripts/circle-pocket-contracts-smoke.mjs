...
assert.match(pocketLandingSource, /<PocketThemeToggle \/>/)
assert.doesNotMatch(pocketLandingSource, /Guest Login|connected wallet/)
assert.match(pocketLandingSource, /Secure email access\./)
...
assert.match(pocketLandingSource, /pointer-events-auto hidden items-center gap-2\.5 sm:flex/)
...
assert.match(pocketSessionSplashSource, /setState\('launching'\), 1_800/)
...
assert.match(circlePocketAppSource, /usePocketSessionSplash\(launchSurface, sessionResolved\)/)
...
assert.match(pocketRouteShellSource, /top: headerHeight > 0 \? headerHeight : 'max\(env\(safe-area-inset-top\), var\(--pocket-status-bar-inset, 0px\)\)'/)
...
assert.match(pocketSendPageSource, /restoreOperations: !requestId && mode === 'address'/)
...
assert.match(telegramPaymentLinksSource, /actionLink: \{ label: 'Open Circle Pocket', url: 'https:\/\/pocket\.hashpaylink\.com\/move\/usdc' \}/)
