...
assert.match(pocketBillsPanelSource, /grid-cols-\[minmax\(112px,0\.72fr\)_minmax\(0,1\.28fr\)\]/)
assert.match(pocketBillsPanelSource, /Network suggested from the number\. Change it if this line was ported\./)
assert.match(pocketBillsPanelSource, /VTpass simulates \{billName\} delivery using its official test account/)
assert.match(pocketBillsPanelSource, /disabled=\{locked \|\| bills\.environment === 'sandbox'\}/)
...
assert.match(pocketX402ControllerSource, /reason\.reason === 'wallet_ownership_mismatch'/)
