...
assert.match(pocketWalletControllerSource, /\.catch\(\(\) => null\)/)
...
assert.match(mainSource, /import '@fontsource\/inter\/latin\.css'/)
...
assert.match(pocketBridgeControllerSource, /setStatus\('successful'\)/)
assert.match(pocketBridgeControllerSource, /USDC sent from \$\{bridgeNetworkLabel\(input\.source\)\} · Arriving on/)
...
assert.match(pocketHomePageSource, /label: 'Arc'.*soon: true/)
...
assert.match(pocketEmailLoginSource, /Pocket will never ask you to share this code\./)
...
assert.match(paymentPageSource, /label: value === 'arc' \? 'Arc Test' : CHAIN_META\[value\]\.label/)
...
assert.match(pocketActivityPanelSource, /const collapsible = Boolean\(receipt\)/)
...
assert.match(agentWalletSource, /CIRCLE_GATEWAY_BALANCE_API_TESTNET/)
