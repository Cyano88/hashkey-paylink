...
const pocketQuickApprovalSource = await readFile(new URL('../src/pocket/lib/pocketQuickApproval.ts', import.meta.url), 'utf8')
...
assert.match(pocketQuickApprovalSource, /AccessControl\.BIOMETRY_CURRENT_SET/)
assert.doesNotMatch(pocketQuickApprovalSource, /NativeBiometric\.verifyIdentity/)
assert.match(pocketQuickApprovalSource, /enablePocketQuickApproval\(email: string, session: CircleEvmEmailSession\)/)
assert.match(pocketQuickApprovalSource, /window\.localStorage\.setItem\(ENABLED_KEY, 'true'\)/)
assert.match(pocketQuickApprovalSource, /window\.localStorage\.setItem\(ENABLED_KEY, 'false'\)/)
assert.match(pocketQuickApprovalSource, /getSecureCredentials/)
assert.match(pocketQuickApprovalSource, /title: 'Unlock Pocket'/)
assert.match(pocketQuickApprovalSource, /negativeButtonText: 'Cancel'/)
assert.doesNotMatch(pocketQuickApprovalSource, /negativeButtonText: 'Use email code'/)
assert.match(pocketQuickApprovalSource, /Pocket unlock was cancelled\./)
assert.match(pocketQuickApprovalSource, /throw new PocketBiometricApprovalError/)
assert.match(pocketQuickApprovalSource, /refreshCircleEvmEmailSession/)
assert.match(pocketWalletControllerSource, /if \(pocketQuickApprovalEnabled\(\)\) \{[\s\S]*readPocketEvmQuickSession/)
assert.match(pocketWalletControllerSource, /resumeCircleSolanaEmailWallet/)
assert.match(pocketWalletControllerSource, /readPocketQuickApprovalSession/)
assert.match(pocketQuickApprovalSource, /session = await refreshCircleEvmEmailSession\(storedSession\)/)
assert.match(pocketQuickApprovalSource, /await encryptSession\(normalizedEmail, secret, session\)/)
assert.match(pocketQuickApprovalSource, /window\.crypto\.subtle\.encrypt/)
assert.match(pocketQuickApprovalSource, /const activeSessionSecrets = new Map/)
assert.doesNotMatch(pocketQuickApprovalSource, /tokenIsUsable/)
assert.match(pocketQuickApprovalSource, /productionEvmTopology\?\.wallets\?\.\[network\]/)
assert.doesNotMatch(pocketQuickApprovalSource, /UNLOCK_WINDOW_MS|const unlocked =/)
assert.match(pocketWalletControllerSource, /evmSessionRef\.current = session/)
assert.doesNotMatch(pocketQuickApprovalSource, /console\.|sessionStorage/)
...
assert.match(pocketRouteShellSource, /minHeight: \`calc\(100dvh - \$\{headerHeight\}px\)\`, paddingTop: 16/)
...
assert.match(circlePocketAppBootSource, /Use fingerprint or face/)
assert.match(circlePocketAppBootSource, /Use email code/)
assert.match(circlePocketAppBootSource, /disablePocketQuickApproval\(email\)/)
