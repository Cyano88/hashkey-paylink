...
assert.match(pocketBillsPanelSource, /VTpass simulates \{billName\} delivery using its official test account/)
...
assert.match(circlePocketAppBootSource, /prefetchPocketWalletSnapshot\(\{ email, getAccessToken \}\)\.catch\(\(\) => undefined\)/)
...
assert.match(pocketMoveBankPageSource, /\{authenticated && <fieldset disabled=\{mode === 'withdraw' && directLocked\}/)
...
assert.match(pocketTopSwitchSource, /bg-\[#ffffff\]/)
...
assert.match(paymentPageSource, /<span>Prepare Naira payout<\/span>/)
...
assert.match(pocketActivityPanelSource, /const collapsible = kind === 'purchases' \|\| kind === 'bank'/)
