...
## Change
Agent Hash creates shareable links through /api/pocket/paylinks using pocketApiUrl, which targets the Pocket HTTPS origin in native Capacitor and a relative URL on web. Direct user-to-user /api/pocket/requests is a different flow and remains unchanged.

The implementation now lives in api/pocket/paylinks.ts. /api/telegram-request reexports the same handler for installed clients and other legacy callers. Both routes have strictLimiter and share the module-level store mutation queue. Existing request IDs, event IDs, URLs, public GET behavior, authentication, action journal and response shape remain identical. Source equality was verified apart from relative import paths.
