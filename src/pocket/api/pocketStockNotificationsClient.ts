...
export async function stockNotificationsRequest(getAccessToken:()=>Promise<string|null>,body?:Record<string,unknown>,activity=false):Promise<StockInbox>{
 ...
 const request=async()=>{const response=await fetch(pocketApiUrl('/api/pocket/xstocks/notifications'+(activity?'?activity=1':'')),{method:body?'POST':'GET',headers:{Authorization:'Bearer '+token,...(body?{'Content-Type':'application/json'}:{})},...(body?{body:JSON.stringify(body)}:{}),signal:AbortSignal.timeout(12000),cache:'no-store'})
 return stockApiResponse(response)}
 if(!body)return stockRead<StockInbox>(token,'notifications:'+activity,request)
