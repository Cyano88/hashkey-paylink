export type PocketScanCode = { kind: 'pos'; id: string; url: string } | { kind: 'checkout'; id: string; attempt: string; url: string }
...
export function pocketScanDestination(raw: string) {
  try { const code=parsePocketScanCode(raw);return '/home/scan?code='+encodeURIComponent(code.url) } catch { return '' }
}
