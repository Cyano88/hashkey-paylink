...
export function pocketWalletPreparationNotice(notice: PocketWalletUpdateNotice, wallets: CirclePocketWallets): PocketWalletUpdateNotice | 'prepare' {
  ...
  const addresses = (['base', 'arbitrum', 'arc'] as const).map(network => wallets[network]?.address?.trim().toLowerCase())
  return addresses.every(Boolean) && new Set(addresses).size > 1 ? 'prepare' : 'hidden'
}