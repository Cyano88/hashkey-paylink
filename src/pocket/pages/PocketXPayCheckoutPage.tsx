...
export default function PocketXPayCheckoutPage(){
 ...
 return <main className="mx-auto w-full max-w-md px-5 py-10 text-gray-950 dark:text-white"><p className="mb-8 text-center text-sm font-bold">Pocket</p>{error?<p role="alert" className="text-center text-sm">{error}</p>:!merchant?<div role="status" aria-label="Loading checkout" className="h-40 animate-pulse rounded-3xl bg-gray-100 dark:bg-white/10"/>:!identity.authenticated?<section className="rounded-3xl border border-gray-100 p-6 dark:border-white/10"><h1 className="text-xl font-bold">{merchant.name}</h1><p className="mt-3 text-sm text-gray-500">Pay with your XStocks balance.</p><button disabled={!identity.ready} onClick={()=>login?.requestLogin()} className="mt-6 min-h-12 w-full rounded-full bg-gray-950 text-sm font-bold text-white disabled:opacity-40 dark:bg-white dark:text-gray-950">Continue with email</button></section>:<PocketPaymentSecurityGate key={identity.user?.id} email={identity.email} getAccessToken={identity.getAccessToken}><Payment/></PocketPaymentSecurityGate>}<p className="mt-8 text-center text-[11px] text-gray-400">Hash PayLink checkout</p></main>
}
