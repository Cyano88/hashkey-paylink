...
export default function PocketNotificationsPage() {
  ...
  return <div ref={scrollerRef} onTouchStart={startPull} onTouchMove={movePull} onTouchEnd={finishPull} onTouchCancel={() => { pullStartY.current = null; if (!refreshing) { pullDistanceRef.current = 0; setPullDistance(0) } }} className="fixed inset-0 z-[45] overflow-y-auto overscroll-y-contain bg-[#F5F5F7] text-gray-950 dark:bg-black dark:text-white">
    <div aria-hidden="true" className="pointer-events-none fixed inset-x-0 top-[max(.5rem,var(--pocket-safe-top))] z-[60] flex justify-center transition-opacity duration-150" style={{ opacity: pullDistance > 4 || refreshing ? 1 : 0, transform: `translateY(${Math.max(0, pullDistance - 30)}px)` }}><span className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-gray-500 shadow-sm ring-1 ring-gray-200/70 dark:bg-[#171717] dark:text-gray-300 dark:ring-white/10"><Loader2 className="h-6 w-6 animate-spin" style={{ animationPlayState: refreshing ? 'running' : 'paused' }} /></span></div>
    <main className="mx-auto min-h-full w-full max-w-[480px] px-5 pb-[max(2rem,var(--pocket-safe-bottom))] pt-[max(1rem,var(--pocket-safe-top))]"><PocketFlowHeader title="Notifications" onBack={() => navigate(POCKET_BASE_PATH + POCKET_ROUTES.home)} />
      {busy && !items.length ? <PocketNotificationsSkeleton /> : error && !items.length ? <section className="mt-20 text-center"><AlertCircle className="mx-auto h-7 w-7 text-gray-300" /><p className="mt-4 text-sm font-bold">Notifications could not load</p><p className="mx-auto mt-2 max-w-xs text-xs leading-5 text-gray-400">{error}</p><button type="button" onClick={() => void load({ showBusy: true, markRead: true })} className="mt-5 min-h-11 rounded-full bg-gray-950 px-6 text-xs font-semibold text-white dark:bg-white dark:text-gray-950">Try again</button></section> : items.length ? <section className="mt-7 space-y-3">{items.map(item => <article key={item.id} className="rounded-[24px] bg-white p-4 shadow-sm dark:bg-[#121212] dark:shadow-none">
        ...
        {item.direction === 'incoming' && item.status === 'pending' && <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => void decide(item.id, 'decline')} disabled={Boolean(acting)} className="flex min-h-12 items-center justify-center gap-2 rounded-full border border-gray-200 text-xs font-bold disabled:opacity-50 dark:border-[#262626]"><XCircle className="h-4 w-4" />Decline</button><button type="button" onClick={() => void decide(item.id, 'accept')} disabled={Boolean(acting)} className="flex min-h-12 items-center justify-center gap-2 rounded-full bg-gray-950 text-xs font-bold text-white disabled:opacity-50 dark:bg-white dark:text-gray-950"><Check className="h-4 w-4" />Accept</button></div>}
