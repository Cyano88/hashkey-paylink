...
export default function PocketMoveUsdcPage() {
  ...
  return <PocketRouteShell active="home" onSelect={selectNav}>
    ...
    <div className="space-y-3.5">
      ...
      <section className="space-y-3.5 rounded-[24px] border border-gray-200/80 bg-white p-4 shadow-[0_12px_34px_rgba(15,23,42,0.07)] dark:border-[#262626] dark:bg-[#121212] dark:shadow-none">
        <div>
        ...
        {showSignIn ? <div className="overflow-hidden rounded-[22px] bg-[#F5F5F7]/95 p-2 dark:bg-[#151518]/95">
          ...
          {flow === 'collection' && <div className="grid grid-cols-2 gap-1 rounded-2xl bg-gray-100 p-1 dark:bg-[#171717]">
          ...
          {flow === 'collection' && collectionRail === 'local' ? <div className="space-y-2">
            ...
            {[['Ghana', 'GHS'], ['Kenya', 'KES']].map(([country, currency]) => <button key={country} type="button" disabled className="flex min-h-14 w-full items-center justify-between rounded-2xl border border-gray-200 px-4 text-left opacity-55 dark:border-[#262626]"><span><span className="block text-sm font-bold">{country}</span><span className="mt-0.5 block text-[11px] text-gray-400">{currency} local-currency collection</span></span><span className="rounded-full bg-gray-100 px-2.5 py-1 text-[9px] font-black uppercase text-gray-500 dark:bg-white/[0.08]">Soon</span></button>)}
