    ...
    <main className="mx-auto flex min-h-full w-full max-w-[480px] flex-col px-5 pb-[calc(7.5rem+var(--pocket-safe-bottom))] pt-[max(1rem,var(--pocket-safe-top))]">
      ...
      <section className="flex flex-1 flex-col pt-8">
        ...
        {editing ? <div className="mt-8 space-y-5">
          ...
          <button type="button" onClick={() => navigate(POCKET_BASE_PATH + POCKET_ROUTES.verifyName)} className="flex w-full items-center gap-3 rounded-[22px] bg-white p-4 text-left shadow-sm dark:bg-[#121212] dark:shadow-none"><span className="min-w-0 flex-1"><span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.18em] text-gray-400">Bank account name</span><span className="mt-1 block text-sm font-bold">{current?.resolvedName || 'Verify bank name'}</span></span><ChevronRight className="h-4 w-4 text-gray-400" /></button>
