...
import { ArrowLeft, Check, Copy, Loader2, LogOut, Pencil } from '../components/PocketIcons'
...
export default function PocketProfilePage() {
  ...
  return <div className="fixed inset-0 z-[45] overflow-y-auto bg-[#F5F5F7] text-gray-950 dark:bg-[#0A0A0A] dark:text-white">
    <main className="mx-auto flex min-h-full w-full max-w-[480px] flex-col px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))]">
      ...
      <section className="flex flex-1 flex-col pt-8">
        ...
        {editing ? <div className="mt-8 space-y-5">
          ...
          <div className="rounded-[22px] bg-white p-4 shadow-sm dark:bg-white/[0.05]"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-gray-400">Verified name</p><p className="mt-1 text-sm font-bold">{current?.resolvedName || 'Added after bank verification'}</p></div>
          <div className="rounded-[22px] bg-white p-4 shadow-sm dark:bg-white/[0.05]"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-gray-400">Email</p><p className="mt-1 truncate text-sm font-bold">{email}</p></div>
