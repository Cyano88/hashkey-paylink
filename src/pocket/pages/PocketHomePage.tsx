...
export default function PocketHomePage() {
  ...
  return <PocketRouteShell active="home" onSelect={tab => open(navPath(tab))}>
    ...
    <section className="pt-2">
      <div className="flex items-center justify-between"><div><p className="text-sm font-black text-gray-950 dark:text-white">Recent activity</p></div><button type="button" onClick={() => open(POCKET_ROUTES.activity)} className="flex items-center gap-1 text-[11px] font-bold text-gray-500">View all<ChevronRight className="h-3.5 w-3.5" /></button></div>
      <div className="mt-3">{!activityReady ? <PocketRecentActivitySkeleton /> : recent.length ? recent.map(row => <button key={row.eventId + ':' + row.txHash} type="button" onClick={() => setSelectedActivity(row)} className="relative flex w-full items-center gap-3 py-4 text-left after:absolute after:bottom-0 after:left-12 after:right-0 after:h-px after:bg-gray-200/70 last:after:hidden dark:after:bg-white/[0.07]"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-white/[0.07]">{row.direction === 'in' || ['refunded', 'reversed'].includes(String(row.paycrestStatus ?? '').toLowerCase()) || row.refundTxHash ? <ArrowDownToLine className="h-4 w-4" /> : <ArrowUpFromLine className="h-4 w-4" />}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-bold text-gray-900 dark:text-white">{pocketBankRecipientLabel(row) || row.activityLabel || row.memo || 'Payment'}</span><span className="mt-0.5 block text-[10px] text-gray-400">{pocketBankRecipientLabel(row) ? (row.bankName ? row.bankName + ' / Bank transfer' : 'Bank transfer') : new Date(row.ts).toLocaleDateString()}</span></span><span className="text-xs font-black tabular-nums text-gray-900 dark:text-white">{row.direction === 'in' || ['refunded', 'reversed'].includes(String(row.paycrestStatus ?? '').toLowerCase()) || row.refundTxHash ? '+' : '-'}{pocketActivityAmount(row)}</span></button>) : <p className="py-8 text-center text-xs font-medium text-gray-400">{activity.error || 'Your completed payments will appear here.'}</p>}</div>
    </section>
