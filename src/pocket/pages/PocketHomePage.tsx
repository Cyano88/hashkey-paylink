...
export default function PocketHomePage() {
  ...
  return <PocketRouteShell active="home" onSelect={tab => open(navPath(tab))}>
    ...
    <section className="rounded-[24px] border border-gray-100 bg-white p-4 shadow-sm dark:border-white/[0.07] dark:bg-white/[0.035]">
      ...
      <div className="mt-4 space-y-1">{!activityReady ? <PocketRecentActivitySkeleton /> : recent.length ? recent.map(row => <button key={row.eventId + ':' + row.txHash} type="button" onClick={() => open(POCKET_ROUTES.activity)} className="flex w-full items-center gap-3 rounded-2xl px-2 py-3 text-left hover:bg-gray-50 dark:hover:bg-white/[0.04]"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-gray-100 dark:bg-white/[0.07]">{row.direction === 'in' || String(row.paycrestStatus ?? '').toLowerCase() === 'refunded' || row.refundTxHash ? <ArrowDownToLine className="h-4 w-4" /> : <ArrowUpFromLine className="h-4 w-4" />}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-bold text-gray-900 dark:text-white">{row.activityLabel || row.memo || 'Payment'}</span><span className="mt-0.5 block text-[10px] text-gray-400">{new Date(row.ts).toLocaleDateString()}</span></span><span className="text-xs font-black tabular-nums text-gray-900 dark:text-white">{row.direction === 'in' || String(row.paycrestStatus ?? '').toLowerCase() === 'refunded' || row.refundTxHash ? '+' : '-'}{row.amount} USDC</span></button>) : <p className="py-8 text-center text-xs font-medium text-gray-400">{activity.error || 'Your completed payments will appear here.'}</p>}</div>
    </section>
  </PocketRouteShell>
}
