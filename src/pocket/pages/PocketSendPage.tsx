...
export default function PocketSendPage() {
  ...
  const [requestCheckedHash, setRequestCheckedHash] = useState('')
  ...
  useEffect(() => {
    if (!requestId || !send.txHash || !['successful','submitted'].includes(send.status)) return
    setPaymentTxHash(send.txHash)
    window.localStorage.setItem(`pocket.request.payment.${requestId}`, send.txHash)
  }, [requestId, send.txHash, send.status])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if(!cancelled)setRequestCheckedHash(paymentTxHash)
    })()
    return () => { cancelled = true }
  ...
  const transactionState = paymentRequest && requestConfirmed ? 'successful' : send.error && send.status==='idle' ? 'failed' : send.status === 'successful' && !paymentRequest ? 'successful' : send.status === 'pending' || send.status === 'submitted' || requestAccepted || Boolean(paymentTxHash) ? 'pending' : 'failed'
  const requestChecking = Boolean(paymentRequest && !requestConfirmed && (paymentTxHash || send.txHash) && requestCheckedHash !== (paymentTxHash || send.txHash) && !(send.status === 'idle' && send.error))
  ...
  return <PocketRouteShell active="home" onSelect={tab => navigate(POCKET_BASE_PATH + navPath(tab))}>
    ...
    <section className="space-y-4 rounded-[26px] border border-gray-100 bg-white p-5 shadow-sm dark:border-[#262626] dark:bg-[#121212] dark:shadow-none">
      ...
      <button type="button" className="min-h-12 w-full rounded-xl bg-black text-sm font-bold text-white disabled:opacity-40" disabled={!recipientReady || !send.amount || resolving || reviewOpen || resultOpen} onClick={async()=>{setResultDismissed(false);setAttemptedAt(0);setReviewOpen(true);setReviewPreparing(true);try{if(!paymentRequest)await send.prepare()}catch{/* Preparation errors stay inside the review. */}finally{setReviewPreparing(false)}}}>Review send</button>
      ...
      {send.notice && !reviewOpen && !resultOpen && <p className="text-center text-xs font-semibold text-emerald-600">{send.notice}</p>}
      {!reviewOpen && !resultOpen && (requestError || send.error || wallets.error || requestLiquidity.error) && <p className="rounded-xl bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 dark:bg-red-400/10 dark:text-red-200">{requestError || send.error || wallets.error || requestLiquidity.error}</p>}
    ...
    {reviewOpen && !resultOpen && <PocketBottomSheet title={paymentRequest ? 'Review payment' : 'Review send'} showCloseButton dismissOnBackdrop={false} dismissible={!reviewPreparing && !approvalBusy && send.status === 'idle'} onClose={()=>setReviewOpen(false)}>
      <PocketConfirmationDetails amount={send.amount + ' USDC'} rows={[
        ...
        ...(send.feePreview ? [['Platform fee', send.feePreview.platform + ' USDC'], ['Network estimate', send.feePreview.network + ' USDC'], ['Total', send.feePreview.total + ' USDC']] as Array<[string,string]> : []),
      ]} />
      <div className="mt-5">{requestPaid ? <div className="rounded-2xl bg-emerald-50 px-4 py-4 text-center dark:bg-emerald-400/10"><Check className="mx-auto h-5 w-5 text-emerald-600" /><p className="mt-2 text-sm font-bold">Request paid</p><p className="mt-1 text-[11px] leading-5 text-gray-500 dark:text-gray-400">{requestConfirmed ? 'The sender has been notified and this transfer will appear in Activity.' : 'Your payment was sent successfully. It will appear in Activity shortly.'}</p></div> : <PocketSlideAction plain onApprovalBusyChange={setApprovalBusy} status={reviewPreparing ? 'pending' : requestChecking ? 'submitted' : paymentRequest && (requestLiquidity.status === 'waiting' || requestLiquidity.status === 'reconciling') ? 'submitted' : paymentRequest && requestLiquidity.status === 'moving' ? 'pending' : send.status} disabled={!recipientReady || !send.amount || resolving || Boolean(paymentTxHash) || Boolean(paymentRequest && (requestLiquidity.checking || requestLiquidity.insufficient))} onConfirm={submitPayment} labels={{ disabled: paymentTxHash ? 'Confirming payment' : paymentRequest && requestLiquidity.checking ? 'Checking balance' : paymentRequest && requestLiquidity.insufficient ? 'Insufficient USDC' : recipientReady ? 'Enter amount' : 'Choose recipient', idle: paymentRequest ? 'Confirm payment' : mode === 'address' && !send.feePreview ? 'Review fees' : 'Confirm send', pending: reviewPreparing ? 'Checking fees' : requestLiquidity.status === 'moving' ? 'Moving USDC' : 'Confirming send', submitted: requestLiquidity.status === 'waiting' || requestLiquidity.status === 'reconciling' ? 'USDC moving' : requestChecking ? 'Confirming payment' : 'Transfer submitted', successful: paymentRequest ? 'Request paid' : 'Sent' }} />}</div>
      {(send.error || requestLiquidity.error) && <p role="alert" className="mt-3 text-xs text-red-500">{send.error || requestLiquidity.error}</p>}
    </PocketBottomSheet>}
    {resultOpen && <PocketTransactionSheet title={paymentRequest ? 'Request payment' : 'Outgoing'} state={transactionState} amount={(paymentRequest?.amount || send.amount) + ' USDC'} receipt={resultReceipt} detail={transactionState === 'failed' ? send.error : transactionState === 'pending' ? 'Waiting for confirmation. You can check Activity for updates.' : undefined} onDone={() => { setResultDismissed(true); setReviewOpen(false); send.reset(); if (transactionState === 'failed' && requestId) { setPaymentTxHash(''); setRequestAccepted(false); window.localStorage.removeItem(`pocket.request.payment.${requestId}`); window.localStorage.removeItem(`pocket.request.accepted.${requestId}`) } navigate(POCKET_BASE_PATH + POCKET_ROUTES.home) }} />}
  </PocketRouteShell>
}
