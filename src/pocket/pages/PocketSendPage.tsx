...
export default function PocketSendPage() {
  ...
  return <PocketRouteShell active="home" onSelect={tab => navigate(POCKET_BASE_PATH + navPath(tab))}>
    ...
    <section className="space-y-4 rounded-[26px] border border-gray-100 bg-white p-5 shadow-sm dark:border-[#262626] dark:bg-[#121212] dark:shadow-none">
      ...
      <PocketSlideAction plain approvalRequired={false} onPrepare={async()=>{if(!paymentRequest){try{await send.prepare()}catch(error){if(!(error instanceof Error) || error.message !== 'FEE_REVIEW_REQUIRED') throw error}}}} status="idle" disabled={!recipientReady || !send.amount || resolving} onConfirm={()=>{setResultDismissed(false);setReviewOpen(true)}} labels={{idle:'Review send',pending:'Reviewing fees',disabled:recipientReady?'Enter amount':'Choose recipient'}} />
