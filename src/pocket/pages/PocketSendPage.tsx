...
export default function PocketSendPage() {
  ...
  return <PocketRouteShell active="home" onSelect={tab => navigate(POCKET_BASE_PATH + navPath(tab))}>
    ...
    {reviewOpen && !resultOpen && <PocketBottomSheet title={paymentRequest ? 'Review payment' : 'Review send'} showCloseButton dismissOnBackdrop={false} dismissible={!approvalBusy && send.status === 'idle'} onClose={()=>setReviewOpen(false)}>
      ...
      <div className="mt-5">{requestPaid ? <div className="rounded-2xl bg-emerald-50 px-4 py-4 text-center dark:bg-emerald-400/10"><Check className="mx-auto h-5 w-5 text-emerald-600" /><p className="mt-2 text-sm font-bold">Request paid</p><p className="mt-1 text-[11px] leading-5 text-gray-500 dark:text-gray-400">{requestConfirmed ? 'The sender has been notified and this transfer will appear in Activity.' : 'Your payment was sent successfully. It will appear in Activity shortly.'}</p></div> : <PocketSlideAction onApprovalBusyChange={setApprovalBusy} status={paymentRequest && (requestLiquidity.status === 'waiting' || requestLiquidity.status === 'reconciling') ? 'submitted' : paymentRequest && requestLiquidity.status === 'moving' ? 'pending' : send.status} disabled={!recipientReady || !send.amount || resolving || Boolean(paymentTxHash) || Boolean(paymentRequest && (requestLiquidity.checking || requestLiquidity.insufficient))} onConfirm={submitPayment} labels={{ disabled: paymentTxHash ? 'Confirming payment' : paymentRequest && requestLiquidity.checking ? 'Checking balance' : paymentRequest && requestLiquidity.insufficient ? 'Insufficient USDC' : recipientReady ? 'Enter amount' : 'Choose recipient', idle: paymentRequest ? 'Confirm payment' : mode === 'address' && !send.feePreview ? 'Review fees' : 'Confirm send', pending: requestLiquidity.status === 'moving' ? 'Moving USDC' : 'Confirm send in Circle', submitted: requestLiquidity.status === 'waiting' || requestLiquidity.status === 'reconciling' ? 'USDC moving' : 'Transfer submitted', successful: paymentRequest ? 'Request paid' : 'Sent' }} />}</div>
