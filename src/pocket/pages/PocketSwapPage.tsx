...
export default function PocketSwapPage() {
  ...
  return <PocketRouteShell active="home" onSelect={tab => navigate(POCKET_BASE_PATH + navPath(tab))}>
    ...
    <section className="space-y-5 rounded-[26px] border border-gray-100 bg-white p-5 shadow-sm dark:border-white/[0.07] dark:bg-white/[0.035]">
      ...
      <PocketSlideAction status={swap.status === 'successful' ? 'successful' : swap.status === 'bridging' ? 'submitted' : swap.status === 'confirming' ? 'pending' : 'idle'} disabled={!swap.quote || Number(swap.quote.total) > sourceBalance || swap.status === 'quoting'} onConfirm={() => void swap.bridge()} labels={{ disabled: swap.status === 'quoting' ? 'Getting live quote' : 'Enter swap amount', idle: 'Confirm swap', pending: 'Preparing swap', submitted: 'Swapping...', successful: 'Swapped' }} />
