...
export default function PocketArcTokenPicker({ label, value, tokens, excluded, disabled, onChange, discover }: Props) {
  ...
  const listed = normalized ? filtered : [...available].sort((a, b) => Number(Number(b.balance) > 0) - Number(Number(a.balance) > 0)).slice(0, 12)
  useEffect(() => {
    if (!open) return
  ...
  return <>
    ...
    {open && createPortal(<div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/40 sm:items-center" onClick={event => { if (event.target === event.currentTarget) setOpen(false) }}>
      <div ref={root} role="dialog" aria-modal="true" aria-label={label} tabIndex={-1} className="flex max-h-[85dvh] w-full max-w-md flex-col rounded-t-[28px] bg-white p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] text-gray-950 shadow-xl outline-none dark:bg-gray-950 dark:text-white sm:rounded-[28px]">
        ...
        {searching ? <label className="mb-3 flex items-center gap-2 rounded-2xl bg-gray-50 px-3 dark:bg-white/5"><Search className="h-4 w-4 shrink-0 text-gray-400" /><input autoFocus aria-label="Search Arc tokens or paste contract" placeholder="Search name or paste contract" value={query} onChange={event => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent py-3 text-sm outline-none" /></label> : <button type="button" onClick={() => setSearching(true)} className="mb-3 flex items-center gap-2 rounded-2xl bg-gray-50 px-3 py-3 text-left text-sm text-gray-500 dark:bg-white/5"><Search className="h-4 w-4" />Search or paste contract</button>}
        ...
        <div className="min-h-0 overflow-y-auto overscroll-contain">
          ...
          {loading && <p role="status" className="py-6 text-center text-xs text-gray-400">Looking up token on Arc…</p>}
          {error && <p role="alert" className="py-4 text-xs text-red-600">{error}</p>}
          {!listed.length && !found && !loading && !error && <p className="py-6 text-center text-xs text-gray-400">No matching token. Paste its Arc contract address to look it up.</p>}
        </div>
      </div>
    </div>, document.body)}
  </>
