    ...
    <main className="mx-auto flex h-[100dvh] w-full max-w-[480px] flex-col overflow-hidden" style={{paddingTop:'var(--pocket-safe-top)',paddingBottom:'var(--pocket-safe-bottom)'}}>
      {view==='home'? <>
        ...
        <header className="flex min-h-16 shrink-0 items-center justify-between border-b border-gray-100 px-4 dark:border-white/10">
          <button aria-label="Back" onClick={()=>{setView(view==='chat'?(active?.status==='resolved'?'previous':'messages'):view==='previous'?'messages':'home');setError('')}} className="flex h-11 w-11 items-center justify-center"><ArrowLeft className="h-5 w-5"/></button>
          <div className="text-center"><h1 className="text-base font-semibold">{view==='chat'?'Pocket Support':view==='faqs'?'Support':view==='previous'?'Previous conversations':'Messages'}</h1>{view==='chat'&&<p className="text-[11px] text-gray-500">{active?.status==='resolved'?'Conversation closed':active?.humanSupport?'The team will reply here':'The team can also help'}</p>}</div>
