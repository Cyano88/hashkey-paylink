...
export default function PocketReceiptReport({receipt,onClose}:{receipt:PaylinkReceipt;onClose:()=>void}) {
  ...
  const start=async()=>{if(report){view();return}setOpen(true);setError('');setChecking(true);const version=++lookupVersion.current;try{const found=await service.call({action:'transaction-report-status',transaction:selector});if(version===lookupVersion.current)setReport(found)}catch(e){setError(e instanceof Error?e.message:'Transaction could not be loaded.')}finally{setChecking(false)}}
  const submit=async()=>{
  ...
  return <>
    ...
    {open&&<PocketBottomSheet title="Report a problem" layer={160} onClose={()=>setOpen(false)} dismissible={!busy}>
      ...
      {report?<div className="space-y-4 text-center"><p className="text-sm text-gray-500">Your transaction and description are saved for manual review by Pocket Support.</p><p className="text-xs font-mono">Case {report.id}</p><button type="button" onClick={view} className="min-h-12 w-full rounded-full bg-gray-950 text-sm font-semibold text-white dark:bg-white dark:text-gray-950">View report in Agent Hash</button><button type="button" onClick={()=>setOpen(false)} className="min-h-10 px-4 text-sm">Done</button></div>:<form onSubmit={event=>{event.preventDefault();void submit()}} className="space-y-4">
        ...
        <label className="block text-xs font-semibold">Reason<select aria-label="Reason" required value={reason} onChange={event=>setReason(event.target.value)} disabled={busy} className="mt-2 min-h-12 w-full rounded-xl bg-gray-100 px-3 text-sm font-normal dark:bg-white/10"><option value="">Select a reason</option>{pocketReportReasons.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>
        <label className="block text-xs font-semibold">Describe the problem<textarea aria-label="Describe the problem" required minLength={10} maxLength={1500} rows={4} value={description} disabled={busy} onChange={event=>setDescription(event.target.value)} placeholder="Describe what happened and what you expected." className="mt-2 w-full resize-none rounded-xl bg-gray-100 p-3 text-sm font-normal dark:bg-white/10"/></label>
        ...
        <button type="button" disabled={busy} onClick={()=>setOpen(false)} className="min-h-10 w-full text-sm text-gray-500">Cancel</button>
      </form>}
    </PocketBottomSheet>}
  </>
}
