...
export default function PocketPreviousWallets({email,getAccessToken,onBack}:{email:string;getAccessToken():Promise<string|null>;onBack():void}) {
 ...
 async function review(network:string) {
  ...
  try {
   ...
   const result=await pocketMigrationRequest<{empty?:boolean}>(token,{action:'review',network,userToken:unlocked.session.userToken},{recovery:true})
   if(!active.current)return
   if(result.empty)setMessage('No USDC is waiting in this previous wallet.')
   else setSession(unlocked.session)
 ...
 return <div className='space-y-4'>
  ...
  {complete?<p className='text-sm text-gray-500'>Your USDC transfer to your updated wallet is confirmed.</p>:session?<PocketMigrationExecution recovery session={session} getAccessToken={getAccessToken} onComplete={()=>setComplete(true)}/>:<>
   <p className='text-sm leading-6 text-gray-500'>Check for USDC sent to a previous wallet after your update.</p>
   {(['base','arbitrum','arc'] as const).map(network=><button key={network} type='button' disabled={busy} onClick={()=>void review(network)} className='min-h-12 w-full rounded-full bg-gray-100 px-4 text-sm font-bold capitalize disabled:opacity-50 dark:bg-white/10'>{network==='arbitrum'?'Arbitrum':network==='arc'?'Arc':'Base'}</button>)}
  </>}
