...
export function stockActivityRows(inbox:StockInbox|null,payments:XPayPayment[],address:string):PocketActivityRow[]{
 ...
 const transfers=(inbox?.notices||[]).flatMap(n=>{const t=n.transfer;if(!t||!n.hash||xpayHashes.has(n.hash.toLowerCase()))return [];return [{eventId:n.id,txHash:n.hash,chain:'xlayer',payer:t.from,recipient:t.to,destination:t.to,memo:t.symbol+(t.direction==='in'?' received':' sent'),amount:t.amount,assetSymbol:t.symbol,ts:n.at,source:'wallet-transfer',settlementType:'wallet_transfer',direction:t.direction,paycrestStatus:'successful'}] as PocketActivityRow[]})
 ...
 const requests=(inbox?.requests||[]).filter(r=>r.status!=='paid').map<PocketActivityRow>(r=>({eventId:r.id,txHash:r.txHash||'',chain:'xlayer',payer:'ID:'+r.payerPocketId,recipient:'ID:'+r.senderPocketId,memo:'Request · '+r.symbol,amount:r.amount,assetSymbol:r.symbol,ts:r.at,source:'request',direction:r.direction==='incoming'?'out':'in',paycrestStatus:r.status==='declined'?'cancelled':r.status==='accepted'?'pending':'awaiting response'}))
...
export default function PocketStockActivity({wallet,payments:provided,historyOnly=false}:{wallet:ReturnType<typeof usePocketStockWallet>;payments?:XPayPayment[];historyOnly?:boolean}){
 ...
 useEffect(()=>{if(historyOnly){setBusy(false);return}let cancelled=false,reading=false;setInbox(null);setPayments([]);setBusy(true);setError('')
  const refresh=async()=>{if(reading||!authenticated||document.visibilityState==='hidden')return;reading=true;try{const [notices,xpay]=await Promise.all([stockNotificationsRequest(getAccessToken,undefined,true),xpayRequest(getAccessToken,{action:'mine'})]);if(!cancelled){setInbox(notices);setPayments(xpay.payments||[]);setError('')}}catch(e){if(!cancelled)setError(e instanceof Error?e.message:'Activity could not load.')}finally{reading=false;if(!cancelled)setBusy(false)}}
