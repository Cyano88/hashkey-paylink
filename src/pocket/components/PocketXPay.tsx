...
export default function PocketXPay({wallet,checkout=false}:{wallet:ReturnType<typeof usePocketStockWallet>;checkout?:boolean}){
 ...
 return <>
  ...
  {open&&(payment?.status==='paid'?<PocketPaymentSuccess receipt={xpayReceipt(payment)} onDone={close} inline={checkout}/>:<Surface title="XPay" onClose={close} showCloseButton dismissible={!busy} dismissOnBackdrop={false}>
