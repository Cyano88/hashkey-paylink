...
export default function PocketBottomSheet({ title, onClose, children, dismissible = true, layer = 85 }: { title: string; onClose: () => void; children: ReactNode; dismissible?: boolean; layer?:number }) {
  ...
  return createPortal(<div style={{zIndex:layer}} className="fixed inset-0 z-[85] flex items-end justify-center bg-black/40 px-0 pt-[max(1rem,var(--pocket-safe-top))]" onClick={event => { if (event.target === event.currentTarget && dismissible) onClose() }}>
