...
export default function PocketHeaderIdentity() {
  ...
  if (!authenticated) return <span className="text-sm font-black tracking-[-0.025em]">ID:—</span>
  ...
  return <>
    <button type="button" onClick={event => { event.preventDefault(); event.stopPropagation(); void copyId() }} title={pocketId} className="max-w-full truncate text-[10px] font-bold tabular-nums tracking-tight" aria-label="Copy full Pocket ID">ID:{pocketId ? pocketId.slice(0, 8) : '—'}</button>
