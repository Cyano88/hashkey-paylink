...
export default function PocketWalletPreparation({ email, getAccessToken }: {
  ...
  const goHome = () => navigate(POCKET_BASE_PATH + POCKET_ROUTES.home, {replace:true})
  useEffect(() => {
    active.current=true
    void (async()=>{
  ...
  return <section className='w-full space-y-4 pt-6'>
    <article className='w-full rounded-[26px] bg-white p-5 shadow-sm dark:bg-white/[0.05]'>
      ...
      {stage==='completed' ? <><p className='mt-3 text-sm leading-6 text-gray-500'>Your balance migration and updated wallets are confirmed.</p><button type='button' onClick={goHome} className={cta}>Proceed to Pocket</button></> : <>
        ...
        {stage==='review' && review && <>
          ...
          <p role='status' className='mt-5 text-sm leading-6 text-gray-500'>USDC balances only. Transfers and fee review are not enabled yet. No funds have moved.</p>
        </>}
        <button type='button' onClick={()=>void run()} disabled={busy} className={cta}>{busy?'Checking�':stage==='verified'?'Review balance migration':stage==='review'?'Refresh balances':'Verify updated wallets'}</button>
