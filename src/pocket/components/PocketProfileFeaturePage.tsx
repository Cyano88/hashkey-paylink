...
function RatesPanel({ fx, currency, onCurrency }: { fx: ReturnType<typeof usePocketFxQuote>; currency: string; onCurrency(value: string): void }) {
  ...
  return <section className='pt-10'>
    ...
    <div className='mt-3 rounded-[28px] bg-white p-5 shadow-sm dark:bg-[#121212] dark:shadow-none'>
      <div className='flex items-center justify-between border-b border-gray-100 pb-5 dark:border-[#262626]'>
      ...
      <div className='pt-5'>
        ...
        <select id='pocket-rate-currency' value={currency} onChange={event => onCurrency(event.target.value)} className='mt-2 min-h-12 w-full rounded-2xl bg-gray-50 px-4 text-sm font-bold outline-none dark:bg-[#171717]'>
        ...
        <button type='button' onClick={() => void fx.refresh()} disabled={fx.busy} className='mt-5 min-h-11 w-full rounded-full border border-gray-200 text-xs font-black disabled:opacity-50 dark:border-[#262626]'>Refresh rate</button>
      </div>
    </div>
  </section>
}
...
function LimitProgress({ title, used, limit, detail }: { title: string; used: number | null; limit: number; detail: string }) {
  ...
  return <article className='rounded-[24px] bg-white p-5 shadow-sm dark:bg-[#121212] dark:shadow-none'>
...
function NotificationsPanel({ enabled, onChange }: { enabled: boolean; onChange(value: boolean): void }) {
  return <section className='pt-10'>
    <div className='rounded-[26px] bg-white p-5 shadow-sm dark:bg-[#121212] dark:shadow-none'>
