    ...
    <div data-pocket-app-frame={isPocketAppPage ? 'true' : undefined} className={`${isPocketAppPage ? 'h-[100dvh] min-h-0 overflow-hidden' : 'min-h-screen'} bg-[#F5F5F7] dark:bg-[#0A0A0A] font-sans flex flex-col`}>
      ...
      <header
        ...
        style={agentHashComposerFocused && agentHashViewportTop > 0
          ...
          : undefined}
        className={isPocketLandingPage || isPocketImmersivePage || isPayPage || isNgPosPage
          ? 'hidden'
          : isPocketAppPage
      ...
      >
        <div
          className={`relative mx-auto flex max-w-5xl items-center px-4 sm:px-6 ${isPocketAppPage ? 'flex-col gap-1.5 pb-2' : `justify-between ${isPolyDeskSurface ? 'pt-3 pb-2' : 'py-3'}`}`}
          style={isPocketAppPage ? { paddingTop: 'calc(max(env(safe-area-inset-top), var(--pocket-status-bar-inset, 0px)) + 0.5rem)' } : undefined}
        >
          {isPocketAppPage ? (
            <>
      ...
      <footer
        ...
        className={`h-[60px] shrink-0 items-center border-t border-gray-100 bg-white py-0 dark:border-white/5 dark:bg-[#111113] ${agentHashComposerFocused || isPocketAppPage || isPayPage || isNgPosPage ? 'hidden' : 'flex'}`}
      >
