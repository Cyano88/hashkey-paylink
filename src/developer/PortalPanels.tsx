...
export function ProductsPanel({project,onConfigure}:{project:ProjectSummary;onConfigure:()=>void}) {
  const products=[
    ...
    {id:'xstocks_agreements',name:'xStocks',copy:'Use hosted stock payments and swaps on X Layer.',rule:'Add to an existing configured product. Separate scoped keys and project activation are required. Selecting this does not enable funding or bridging.'},
  ...
  ].filter(p=>project.checkoutMode==='human'||!['polymarket_funding','xstocks_agreements'].includes(p.id))
