...
import { TRADE_XLAYER_ARBITER, type TradeXLayerAction, type TradeXLayerStatus, type TradeXLayerTransaction } from '../../lib/xstocksAgreement/protocol';
...
export default function HostedWorkCheckout({item,request,onUpdated}:{item:ServiceRequest;request(payload:Record<string,unknown>):Promise<unknown>;onUpdated():void}){
  ...
  async function run(action:TradeXLayerAction|'wallet'|'recover'){
    ...
    try{
      if(action==='wallet'){if(!wallet)throw Error('Your embedded wallet is not ready.');if(!await confirm({title:'Accept Agreement terms?',description:terms.amount+' '+workPaymentLabel(payment)+'. '+workTermsNotice(terms.durationSeconds,payment),action:'Accept terms'}))return;assertCurrent();await api({action:'work_xlayer_wallet',address:wallet.address});}
  ...
  return <section className='mt-4 space-y-3 border-t border-gray-200 pt-4 dark:border-white/10' aria-label='Work escrow'>
    ...
    <p className='text-xs text-gray-500'>{status?.pending?'Waiting for network confirmation':status?.state!==undefined?WORK_STATES[status.state]:status?'Accept the Agreement terms to prepare escrow.':'Checking Agreement...'}</p>
