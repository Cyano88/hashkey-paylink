...
function targetVariable(target) {
  ...
  if (![VARIABLE, AGREEMENT_VARIABLE, FUNDING_VARIABLE, CONNECTION_VARIABLE].includes(variable)) throw safeError('Unsupported backend variable.')
  return variable
}
function requireAgreementKey(target, key) {
  const variable = targetVariable(target)
  if (variable === FUNDING_VARIABLE || variable === CONNECTION_VARIABLE) {
    const required = variable === FUNDING_VARIABLE ? ['agreement:recipient', 'agreement:fund'] : ['wallet:connect']
...
function validateTarget(options) {
  ...
  if (options.product !== undefined && !['checkout', 'agreement', 'agreement-funding', 'wallet-connection'].includes(options.product)) throw safeError('Choose checkout, agreement, agreement-funding or wallet-connection.')
  if (['agreement', 'agreement-funding', 'wallet-connection'].includes(options.product)) {
    if (options.provider !== 'render') throw safeError('Agreement handoff currently supports Render only.')
    target.variable = options.product === 'agreement-funding' ? FUNDING_VARIABLE : options.product === 'wallet-connection' ? CONNECTION_VARIABLE : AGREEMENT_VARIABLE
  }
  if (target.provider === 'render') {
