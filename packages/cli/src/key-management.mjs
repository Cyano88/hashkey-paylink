...
export async function keyCommand(command, options, { fetcher, sessionStore, vaultStore }) {
  ...
  if (!/^[a-zA-Z0-9:_-]{16,128}$/.test(operationId ?? '') || !name?.trim() || name.length > 60
    ...
    || scopes.some(scope => !['project:read','checkout:read','checkout:create','agreement:read','agreement:create','xstocks-agreement:read','wallet:connect', 'xstocks-agreement:create'].includes(scope) || !session.grant.scopes.includes(scope))) throw safeError('Invalid key inputs or permissions exceed the approved grant.')
