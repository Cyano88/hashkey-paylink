...
`;var ie=({iconUrl:a,value:l,symbol:o,usdValue:m,nftName:E,nftCount:p,decimals:t,$isLoading:k})=>{if(k)return e.jsx(te,{$isLoading:k});let y=l&&m&&t?function(b,I,A){let w=parseFloat(b),x=parseFloat(A);if(w===0||x===0||Number.isNaN(w)||Number.isNaN(x))return b;let g=Math.ceil(-Math.log10(.01/(x/w))),d=Math.pow(10,g=Math.max(g=Math.min(g,I),1)),T=+(Math.floor(w*d)/d).toFixed(g).replace(/\.?0+$/,"");return Intl.NumberFormat(void 0,{maximumFractionDigits:I}).format(T)}(l,t,m):l;return e.jsxs("div",{children:[e.jsxs(te,{$isLoading:k,children:[a&&e.jsx(ir,{src:a,alt:"Token icon"}),p&&p>1?p+"x":void 0," ",E,y," ",o]}),m&&e.jsxs(nr,{$isLoading:k,children:["$",m]})]})};let te=h.span`
