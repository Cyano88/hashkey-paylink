...
import pocketPaylinkRequestsHandler from './api/pocket/paylink-requests.js'
...
import pocketEvmTransferStatusHandler from './api/pocket/evm-transfer-status.js'
import {
...
import pocketAdditionalMigrationHandler from './api/pocket/wallet-additional-flow.js'
import pocketMigrationFlowHandler from './api/pocket/wallet-migration-flow.js'
...
import developerCliKeysHandler from './api/developer-cli-keys.js'
...
// Keep third-party identity components isolated from Pocket's app origin.
app.get('/pocket/identity-frame', (req, res) => {
...
app.use('/api', (req, res, next) => {
  ...
  if (usesScoped && req.path !== '/v2/cli/auth' && !cliRequestScope(req)) {
    return res.status(403).json({ ok: false, error: 'Scoped credentials are not permitted on this route.' })
  }
  next()
})
...
const pocketKycReadLimiter = rateLimit({ name: 'pocket-kyc-read', windowMs: 60_000, max: 60 })
const pocketKycSessionLimiter = rateLimit({ name: 'pocket-kyc-session', windowMs: 60_000, max: 10 })
app.all('/api/pocket/kyc', (req, res, next) => (['start', 'resume'].includes(req.body?.action) ? pocketKycSessionLimiter : pocketKycReadLimiter)(req, res, next), pocketKycHandler)
...
const pocketBankReadLimiter = rateLimit({ name: 'pocket-bank-read', windowMs: 60_000, max: 90 })
const pocketBankWriteLimiter = rateLimit({ name: 'pocket-bank-write', windowMs: 60_000, max: 20 })
app.all('/api/pocket/bank-withdraw', (req, res, next) => (['status', 'routeStatus', 'recover'].includes(req.body?.action) ? pocketBankReadLimiter : pocketBankWriteLimiter)(req, res, next), pocketBankWithdrawHandler)
...
app.all('/api/pocket/bridge/solana/prepare', relayLimiter, pocketSolanaCctpPrepareHandler)
...
app.all('/api/pocket/wallet-update/flow', strictLimiter, pocketMigrationFlowHandler)
app.all('/api/pocket/wallet-update/recovery', strictLimiter, pocketMigrationLegacyFlowHandler)
app.all('/api/pocket/wallet-update/additional', strictLimiter, pocketAdditionalMigrationHandler)
...
const walletSwapSessions=createWalletSwapSessionHandlers()
