 ...
 *
 * GET  /api/relay-arbitrum-usdc          -> returns estimated gas reimbursement in USDC (6-dec)
 * POST /api/relay-arbitrum-usdc          -> submits relay tx, returns txHash
 *
 * Required env vars:
...
export default async function handler(req: Request, res: Response) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      return res.json({ ok: true, gasReimbUsdc: gasReimbUsdc.toString() })
    } catch (err) {
      // Return min floor as safe fallback so UI always has a value
      return res.json({ ok: true, gasReimbUsdc: MIN_GAS_REIMB_USDC.toString() })
    }
  }
