...
export async function sendTransactionalEmail(input: SendEmailInput) {
  if (RESEND_API_KEY) {
    ...
    if (!from) throw new Error(`${input.context} email is not configured. Set a verified FROM email.`)
  ...
  if (!SENDGRID_API_KEY || !input.fromEmail) {
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => '')
    throw new Error(body ? `SendGrid rejected ${input.context}: ${body.slice(0, 180)}` : `SendGrid rejected ${input.context} email.`)
  }
}
