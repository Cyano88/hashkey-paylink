...
const ROUTES: Record<CirclePocketCapability, Omit<CirclePocketRoute, 'source' | 'capability' | 'supported' | 'confidence'>> = {
  'wallet-overview': {
    answer: 'Circle Pocket keeps receiving USDC, transfers, bank settlement, POS, bills, and payment history in one place.',
    action: { label: 'Open Circle Pocket', url: 'https://pocket.hashpaylink.com/move/usdc' },
  },
  'receive-usdc': {
  ...
  'profile-support': {
    answer: 'Open Circle Pocket to sign in, review your verified account, edit available profile details, and manage payments.',
    action: { label: 'Open Circle Pocket', url: 'https://pocket.hashpaylink.com/move/usdc' },
  },
}
...
export function routeCirclePocketQuestion(question: string, helperMode: string): CirclePocketRoute | undefined {
  ...
  return {
    ...
    supported: false,
    answer: 'Circle Pocket does not handle that request directly yet. The closest available help is receiving USDC, bank payout, Retail POS, bills, or receipts. Open Circle Pocket to choose the right flow.',
  }
}
