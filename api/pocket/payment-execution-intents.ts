...
function normalizedStore(value?: Partial<Store>): Store { return { intents: value?.intents ?? {}, idempotency: value?.idempotency ?? {}, ledgerOutbox: value?.ledgerOutbox ?? {} } }
...
export function createPaymentExecutionRepository(options: Options = {}) {
  ...
  const appendLedger = options.appendLedger ?? (options.durable === undefined && options.storePath === undefined && options.mutateDurable === undefined ? appendPocketMoneyLedgerEvent : undefined)
  ...
  async function flushLedgerOutbox() {
    ...
    const pending = Object.entries((await readStore()).ledgerOutbox)
    for (const [eventKey, event] of pending) {
      await appendLedger(event)
      await mutate(store => { delete store.ledgerOutbox[eventKey]; return true })
    }
  }
  return {
    ...
    async listOwned(ownerId: string, kinds?: PaymentExecutionKind[], states?: PaymentExecutionState[]) {
      return Object.values((await readStore()).intents)
        .filter(intent => intent.ownerId === ownerId
          && (!kinds?.length || kinds.includes(intent.kind))
          && (!states?.length || states.includes(intent.state)))
        .sort((a, b) => b.updatedAt - a.updatedAt)
    },
    async listUnresolved(limit = 250) {
      const unresolved: PaymentExecutionState[] = ['prepared', 'authorized', 'submitted', 'processing', 'needs_review']
      return Object.values((await readStore()).intents)
        .filter(intent => unresolved.includes(intent.state))
        .sort((a, b) => a.updatedAt - b.updatedAt)
        .slice(0, Math.max(1, Math.min(Math.trunc(limit), 1000)))
    },
