...
export function v3Policy(method:Method){return{country:'NG',countryName:'Nigeria',provider:'smile',policyVersion:'ng-smile-'+method+'-api-v3',method,product:method==='government_id'?'doc_verification':'biometric_kyc',apiProduct:method==='government_id'?'document_verification':'biometric_kyc',idSelection:{NG:method==='government_id'?['PASSPORT','DRIVERS_LICENSE','NATIONAL_ID']:[method==='bvn'?'BVN':'NIN_V2']},consentRequired:{NG:[]},previewBVNMFA:false}}
...
function publicFlow(jobs:Job[],environment:SmileEnvironment){const j=jobs.at(-1),first=bvn(jobs),done=complete(jobs);return{environment,apiVersion:3,status:j?.status||'not_started',jobId:j?.id||null,verification:v3Policy(j?.method||'bvn'),verified:environment==='production'&&!!done,canResume:resumable(j),uploadReported:!!j?.uploadReportedAt,failureReason:j?.failureReason||null,workflow:{bvnPassed:!!first,complete:!!done,needsAdditional:!!first&&!done&&(!j||j.method==='bvn'||j.status==='failed'),methods:['nin','government_id']}}}
...
function evidence(config:SmileConfig,fields:Record<string,unknown>){
 ...
 return{legalName,identityMatch:name&&valid?createHmac('sha256',config.apiKey).update('pocket-identity-v1|'+name+'|'+dob).digest('hex'):undefined}
}
async function refresh(config:SmileConfig,owner:string,j:Job){
 if(!j.providerJobId||['passed','failed'].includes(j.status)||Date.now()-(j.checkedAt||0)<30000)return
 let claimed=false
 ...
 if(!claimed)return
 const result=await smileV3Status(config,j.providerJobId)
 if(result.job_id!==j.providerJobId||result.status!=='not_found'&&result.user_id!==j.providerUserId)throw fail('Verification reference did not match.')
 await mutateDurableJson<Store>(key(owner,config.environment),r=>{const v=r?.jobs.find(x=>x.id===j.id);if(v&&!['passed','failed'].includes(v.status)){v.providerStatus=result.status;if(result.status==='block'||result.status==='error'){v.status='failed';v.failureReason=result.status==='error'?'provider_error':'provider_rejected'}else if(['clear','attention','not_found'].includes(result.status))v.status='review'}return r||{jobs:[]}})
}
export default async function pocketKycV3(req:Request,res:Response){
 ...
 try{
  ...
  if(action==='uploaded'){
   ...
   record=await mutateDurableJson<Store>(k,r=>{const j=r?.jobs.find(x=>x.id===latest!.id);if(j&&!['passed','failed'].includes(j.status))j.uploadReportedAt||=Date.now();return r||{jobs:[]}})
  ...
  const candidate:Job=action==='resume'?latest!:{id:'pkyc_'+randomUUID().replaceAll('-',''),method,environment:config.environment,status:'pending',createdAt:Date.now(),callbackProof:randomBytes(32).toString('hex')}
  ...
  await mutateDurableJson<Store>(k,r=>{const jobs=r?.jobs||[];if(action==='start'){
   ...
   candidate.sessionAt=Date.now();return{jobs:[...jobs,candidate]}
  }const j=jobs.find(x=>x.id===candidate.id);if(!resumable(j)||Date.now()-(j!.sessionAt||0)<20000)throw fail('Your verification is already opening. Please wait.',409);j!.sessionAt=Date.now();return{jobs}})
  ...
  try{const policy=v3Policy(candidate.method),callback=new URL(config.callbackUrl);callback.searchParams.set('reference',candidate.id);callback.searchParams.set('proof',candidate.callbackProof)
 ...
 }catch(error){const status=(error as {status?:number}).status||503;return res.status(status).json({ok:false,error:status<500?(error as Error).message:'Identity verification could not load. Please try again.'})}
}
export async function pocketKycV3Callback(req:Request,res:Response){
 try{
  ...
  if(body.partner_params?.internal_reference!==reference||body.product!==v3Policy(selected.method).apiProduct||selected.providerJobId&&selected.providerJobId!==providerJobId||selected.providerUserId&&selected.providerUserId!==providerUserId)return res.status(409).json({ok:false})
  // Header HMAC authenticates timestamp, not JSON. A provider-only callback proof and
  ...
  const typeMatches=selected.method!=='government_id'||fields.country==='NG'&&v3Policy(selected.method).idSelection.NG.includes(fields.id_type)
