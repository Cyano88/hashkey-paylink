...
export function createWalletConnectionHandlers(overrides: Partial<Deps> = {}) {
  ...
  const developer = wrap(async (req, res) => {
    ...
    if (req.body?.action === 'create') {
      ...
      return res.status(201).json({ ok: true, id, projectName: s.projectName, expiresAt: s.expiresAt, connectPath: `/wallet/connect/${id}#access=${access}` })
    }
    ...
    return res.json({ ok: true, projectName: s.projectName, subject: s.subject, hashPayLinkUserId: s.userId, walletAppId: s.appId })
  })
