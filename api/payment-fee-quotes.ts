...
export function verifyPaymentFeeQuote(token: string, binding: PaymentFeeBinding, now = Date.now(), key = secret()): PaymentFeeQuote {
  ...
  if (quote.walletAddress !== binding.walletAddress.toLowerCase() || quote.recipient !== binding.recipient.toLowerCase()) throw new Error('Payment details changed. Refresh the quote.')
