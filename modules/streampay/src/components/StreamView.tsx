          ...
          {!isCancelled && (
            <div className="space-y-2.5">
              {streamRole !== 'reader' && (() => {
                ...
                const claimTitle = 'Claim with Circle Smart Wallet'
                const claimSubtitle = 'Creators claim streamed USDC with the recipient Circle wallet on Arc'
                const claimPlaceholder = 'email@example.com'
                        ...
                        {!circleWalletMatches && (
                          <div className="rounded-lg border border-red-100 dark:border-red-900/40 bg-red-50 dark:bg-red-950/20 px-2.5 py-2 space-y-2">
                            <p className="text-[11px] font-semibold text-red-500 dark:text-red-300">
                              This wallet is not the stream recipient.
                            </p>
                            <div className="flex items-center justify-between gap-3">
                              <div className="min-w-0">
              ...
              {streamRole !== 'creator' && (() => {
                ...
                return (
                  <div className="rounded-2xl border border-gray-100 dark:border-white/10 bg-gray-50/70 dark:bg-white/[0.04] p-3.5 space-y-3">
                    <div className="min-w-0">
                      <p className="text-[12px] font-bold text-gray-800 dark:text-gray-100">Reader refund wallet</p>
                      <p className="text-[11px] text-gray-500 dark:text-gray-400">End the stream from the sender Circle wallet to recover unstreamed USDC.</p>
                    </div>
                    ...
                    {!senderCircleSession && (
                      <input
                        ...
                        disabled={actionState === 'signing' || actionState === 'relaying' || actionState === 'pending'}
                        className="w-full rounded-xl border-2 border-gray-100 dark:border-white/10 bg-white dark:bg-[#15151a] px-4 py-3 text-[13px] text-gray-800 dark:text-gray-100 placeholder:text-gray-300 dark:placeholder:text-gray-600 focus:outline-none focus:border-blue-300 transition-colors disabled:opacity-50 min-h-[46px]"
                      />
                    )}
                    ...
                    {senderCircleSession && (
                      <div className="rounded-xl border border-gray-100 dark:border-white/10 bg-white dark:bg-[#15151a] px-3 py-2.5 space-y-2">
                        ...
                        {!senderWalletMatches && (
                          <div className="rounded-lg border border-red-100 dark:border-red-900/40 bg-red-50 dark:bg-red-950/20 px-2.5 py-2">
                            <p className="text-[11px] font-semibold text-red-500 dark:text-red-300">
                              This wallet did not start the stream.
                            </p>
                          </div>
                        )}
                      </div>
                    ...
                    {actionKind === 'cancel' && actionState === 'pending' && txHash ? (
                      ...
                      >
                        ...
                        {actionState === 'signing' ? 'Confirm in Circle Smart Wallet' : 'Broadcasting refund to Arc'}
                      </button>
                    ) : (
                      <button
              ...
              {false && !telegramMode && isRecipient && isOnArc && (() => {
                const claimable = stream?.claimable ?? 0n
                const hasBalance = claimable > 0n
              ...
              {/* Sender cancel */}
              {false && !telegramMode && isSender && !isComplete && isOnArc && (
                <button
              ...
              {/* Observer */}
              {false && !telegramMode && !isRecipient && !isSender && isConnected && (
                <a href="/"
